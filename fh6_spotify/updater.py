...
MANIFEST_URL = os.environ.get('SEGUE_MANIFEST_URL', 'https://raw.githubusercontent.com/Segueapp/segue-releases/main/latest.json')
...
def installer_is_directly_updatable(info) -> bool:
    ...
    return bool(getattr(info, 'sha256', '')) and _is_https_exe(getattr(info, 'installer_url', ''))
...
def download_installer(info, on_progress=None) -> str:
    ...
    if os.path.exists(dest) and verify_sha256(dest, info.sha256):
        ...
        return dest
    tmp = dest + '.part'
    req = urllib.request.Request(url, headers={'User-Agent': f'Segue/{VERSION} update-dl'})
...
def apply_update(installer_path: str, quit_fn) -> None:
    ...
    subprocess.Popen([installer_path, '/VERYSILENT', '/UPDATED'], creationflags=flags, close_fds=True)
    quit_fn()
...
def _fetch_manifest() -> Optional[dict]:
    ...
    try:
        ...
        return None
    try:
        req = urllib.request.Request(MANIFEST_URL, headers={'User-Agent': f'Segue/{VERSION} update-check'})
...
def check_async(on_available: Callable[[UpdateInfo], None], force: bool=False, ignore_cooldown: bool=False) -> None:
