...
def build(app: QApplication):
    ...
    try:
        ...
        overlay_srv = StreamOverlayServer(
            ui,
            getattr(c, "stream_overlay_port", 7345),
            on_save=_save_overlay_preset,
            cfg=getattr(c, "overlay_preset", None) or None,
            presets=getattr(c, "overlay_presets", None) or {},
            preset_name=getattr(c, "overlay_preset_name", "") or "",
            on_persist=_save_overlay_presets,
        )
    ...
    try:
        ...
        if not _NO_CAROUSEL and (
            getattr(c, "overlay_video", False) or getattr(c, "connect_skip", False)
        ):
            import subprocess
            import sys as _sys
            ...
            if cmd:
                ...
                _svc_log = open(
                    os.path.join(_log_dir, "canvas_svc.log"),
                    "w",
                    encoding="utf-8",
                    errors="replace",
                )
    ...
    def make_window():
        return SettingsWindow(
            ...
            path=cfg_path,
            on_close=quit_fn,
            on_minimize=lambda: state["window"].hide(),
            ui=ui,
            on_scale=rebuild_window,
    ...
    def _force_foreground(w):
        ...
        attached = (
            bool(tgt) and tgt != our and bool(u.AttachThreadInput(our, tgt, True))
        )
        try:
            SWP = 67
    ...
    app._segue = {
        ...
        "config": c,
        "ui": ui,
        "summon_timer": _summon_timer,
        "show_window": show_window,
        "overlay_srv": overlay_srv,
    }
...
def _install_crash_logger() -> None:
    ...
    def _hook(exc_type, exc, tb):
        _log(
            f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] unhandled:\n"
    ...
    try:
        ...
        def _t_hook(args):
            _log(
                f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] thread {args.thread.name}:\n"
                + "".join(
    ...
    try:
        ...
        def _qt_handler(mode, ctx, msg):
            ...
            line = ctx.line or 0
            _log(
                f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] {label} {file}:{line}: {msg}\n"
            )
    ...
    try:
        ...
        _log(
            f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] asyncio handler setup failed: {e}\n"
        )
