...
def _open_ds4():
    ...
    for product in _DS4_PRODUCTS:
        try:
            return hidapi.Device(vendor_id=_DS4_VENDOR, product_id=product, blocking=False)
...
class DualShockInput:
    ...
    _CAPTURE_BUTTONS = ('cross', 'circle', 'square', 'triangle', 'L1', 'R1', 'L3', 'R3', 'share', 'options', 'touchBtn', 'ps', 'DpadUp', 'DpadDown', 'DpadLeft', 'DpadRight')
        ...
        while not self._stop:
            ...
            try:
                ...
                if self._diag_full is None:
                    try:
                        import os as _os
                        _p = _os.path.join(_os.environ.get('APPDATA', '.'), 'Segue', 'ds4_reports.log')
                        self._diag_full = _os.path.exists(_p) and _os.path.getsize(_p) > 262144
                    except Exception:
                ...
                try:
                    ...
                    if shape not in self._seen_shapes:
                        ...
                        with open(_os.path.join(base, 'ds4_reports.log'), 'a') as f:
                            f.write('report id=0x%02X len=%d head=%s\n' % (data[0], len(data), bytes(data[:16]).hex(' ')))
                except Exception:
                    pass
                try:
            ...
            if self._tp_mode is None:
                if self.c.touchpad_skip_enabled and abs(dx_end) >= intent and abs(dx_end) > abs(dy_end):
                    if dx_end > 0:
                        ...
                        self._on_prev()
                    self._tp_mode = 'skip'
                if self._tp_mode is None and self.c.touchpad_tap_enabled and self.c.pause_input == 'tap' and is_tap((now - self._tp_start_t) * 1000, self._tp_moved, self.c.tap_max_ms, self.c.tap_move_threshold):
                    self._on_tap()
