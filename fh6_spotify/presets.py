...
PRESET_FIELDS = ['full_level', 'menu_level', 'duck_level', 'unfocused_level', 'volume_ramp_in', 'overlay_enabled', 'overlay_position', 'overlay_custom_x', 'overlay_custom_y', 'overlay_scale', 'overlay_compact', 'overlay_screen', 'ducking_enabled', 'low_cpu_mode']
            ...
            for fp in sorted(glob.glob(os.path.join(target, '*.json'))):
                try:
                    with open(fp, 'r', encoding='utf-8') as f:
                        data = json.load(f)
                    if isinstance(data, dict):
                        name = data.get(NAME_KEY) or os.path.splitext(os.path.basename(fp))[0]
                        out[name] = data
                except (OSError, ValueError):
                    pass
        return out
