...
class _THUMBBUTTON(Structure):
    _fields_ = [('dwMask', ctypes.c_uint32), ('iId', ctypes.c_uint32), ('iBitmap', ctypes.c_uint32), ('hIcon', wintypes.HICON), ('szTip', ctypes.c_wchar * 260), ('dwFlags', ctypes.c_uint32)]


class _ICONINFO(Structure):
    _fields_ = [('fIcon', wintypes.BOOL), ('xHotspot', ctypes.c_uint32), ('yHotspot', ctypes.c_uint32), ('hbmMask', wintypes.HBITMAP), ('hbmColor', wintypes.HBITMAP)]


class _BMIH(Structure):
    _fields_ = [('biSize', ctypes.c_uint32), ('biWidth', ctypes.c_int32), ('biHeight', ctypes.c_int32), ('biPlanes', ctypes.c_uint16), ('biBitCount', ctypes.c_uint16), ('biCompression', ctypes.c_uint32), ('biSizeImage', ctypes.c_uint32), ('biXPPM', ctypes.c_int32), ('biYPPM', ctypes.c_int32), ('biClrUsed', ctypes.c_uint32), ('biClrImportant', ctypes.c_uint32)]
...
def _qimage_to_hicon(img):
    ...
    try:
        ...
        gdi.CreateDIBSection.argtypes = [c_void_p, c_void_p, ctypes.c_uint, POINTER(c_void_p), c_void_p, ctypes.c_uint]
        gdi.CreateBitmap.restype = c_void_p
        gdi.CreateBitmap.argtypes = [ctypes.c_int, ctypes.c_int, ctypes.c_uint, ctypes.c_uint, c_void_p]
...
class TaskbarButtons:
    ...
    def __init__(self, hwnd: int, on_action):
        ...
        self._icons = {}
        try:
            self.msg_created = ctypes.windll.user32.RegisterWindowMessageW('TaskbarButtonCreated')
        except Exception:
            self.msg_created = 0
    ...
    def _ensure(self):
        ...
        try:
            ...
            ptr = c_void_p()
            hr = ole32.CoCreateInstance(byref(_guid(_CLSID_TaskbarList)), None, _CLSCTX_INPROC_SERVER, byref(_guid(_IID_ITaskbarList3)), byref(ptr))
            if hr != 0 or not ptr:
                return False
            self._tbl = ptr
    ...
    def _buttons(self):
        ...
        arr[1] = self._mk(BTN_PLAYPAUSE, 'pause' if self._playing else 'play', 'Pause' if self._playing else 'Play')
    ...
    def add(self, src=''):
        ...
        try:
            ...
            if not self._ensure():
                return
            arr = self._buttons()
            hr = self._call(15, ctypes.c_long, [wintypes.HWND, ctypes.c_uint, POINTER(_THUMBBUTTON)], self._hwnd, 3, arr)
            self._added = hr == 0
        except Exception:
            return None
    ...
    def set_playing(self, playing: bool):
        try:
            ...
            if not self._added:
                return
            arr = self._buttons()
            self._call(16, ctypes.c_long, [wintypes.HWND, ctypes.c_uint, POINTER(_THUMBBUTTON)], self._hwnd, 3, arr)
        except Exception:
            return None
    ...
    def handle_command(self, wparam: int) -> bool:
        ...
        if wparam >> 16 & 65535 != _THBN_CLICKED:
            return False
        kind = {BTN_PREV: 'prev', BTN_PLAYPAUSE: 'playpause', BTN_NEXT: 'next'}.get(wparam & 65535)
        if kind:
            try:
