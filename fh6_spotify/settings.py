...
class _ToggleButton(QPushButton):
    ...
    def _retint(self):
        ...
        self.setIcon(
            self._on_icon if self.property("active") == "true" else self._off_icon
        )
...
class _ComboItemDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        if option.state & QStyle.StateFlag.State_Selected:
            ...
            painter.drawText(
                QRectF(option.rect).adjusted(_s(10), 0, -_s(8), 0),
...
class _InfoPopupLabel(QLabel):
    ...
    def enterEvent(self, e):
        ...
        card.setStyleSheet(
            f"QFrame#infopop{{background:{_c('sunk')};border:1px solid {_c('border')};border-radius:{_s(10)}px;}}"
        )
        ...
        pop.adjustSize()
        g = self.mapToGlobal(
            QPoint(self.width() // 2 - pop.width() // 2, self.height() + _s(6))
        )
        pop.move(g)
        pop.show()
        pop.raise_()
...
def _setup_quick_look(
    device: str, mode: str = "forza", preset: str = ""
) -> tuple | None:
    ...
    if device == "playstation":
        if mode == "general":
            ...
            clips = [
                ("Swipe right → next", "clips/touchpad_swipe_right.gif"),
                ("Swipe left → previous", "clips/touchpad_swipe_left.gif"),
                ("Swipe up / down → volume", "clips/touchpad_swipe_up.gif"),
                (pause_cap, "clips/touchpad_tap.gif"),
            ]
        else:
            clips = [
                ("D-pad Left / Right → skip", "clips/dpad_press_right_left.gif"),
                ("Swipe up / down → volume", "clips/touchpad_swipe_up.gif"),
                ("Tap → pause / play", "clips/touchpad_tap.gif"),
            ]
        return (
            "Touchpad controls",
            ["Tap the Controls tab on the left for full details + other devices."],
            [],
            clips,
            2,
        )
    if device == "xbox":
        return (
            "Your controls (quick look)",
            [
                "D-pad Left / Right skips tracks. Everything else (volume, pause) is bindable - see Controls tab."
            ],
            [],
            [("D-pad Left / Right → skip track", "clips/dpad_press_right_left.gif")],
        )
    if device == "wheel":
        return (
            "Your controls (quick look)",
            [
                "If your wheel has a D-pad on the hub, it skips tracks. Bind anything else in Controls → Rebind."
            ],
            [],
            [("D-pad Left / Right → skip track", "clips/dpad_press_right_left.gif")],
        )
...
def _build_howitworks_tab(mode: str = "forza") -> tuple:
    ...
    if mode == "general":
        return (
            ...
            [
                (
                    "Spotify follows your game",
                    [
                        "Segue watches the game you've picked and adjusts Spotify on its own - you don't press anything:",
                        "",
                        "•  Playing the game (window focused) - full volume.",
                        "•  Alt-tabbed / minimized - drops to your Unfocused Volume.",
                        "•  Someone talks in-game (if Speech recognition is on) - ducks down, then back up.",
                        "",
                        "Controllers add manual skip / volume / pause on top - they're optional.",
                    ],
                ),
                (
                    "No telemetry needed",
                    [
                        "General mode works with any game that makes sound - it doesn't read anything from the game itself. Because of that it can't tell an in-game menu from active play the way Forza mode can; it uses which window is focused instead (that's the Unfocused Volume drop when you tab out)."
                    ],
                ),
                (
                    "Spotify or browser",
                    [
                        "Click the source icon in the top bar (next to the link) to switch between Spotify and your browser - handy if you'd rather watch YouTube. Segue then controls that app's volume instead.",
                        "",
                        "Browser note: it dims the whole browser, not one tab, and ads play at full volume.",
                    ],
                ),
            ],
        )
    return (
        ...
        [
            (
                "Spotify becomes your in-game radio",
                [
                    "Segue reads Forza's Data Out feed and sets Spotify's own volume to match what you're doing. You don't press anything - it reacts on its own:",
                    "",
                    "•  Driving - full volume.",
                    "•  Menus / paused - quiet or muted (your Menu Volume).",
                    "•  Someone talks in-game - ducked down (your Ducked Volume), then back up.",
                    "",
                    "Controllers only add manual skip / volume / pause on top of the automatic behaviour - they're optional.",
                ],
            ),
            (
                "Spotify or browser",
...
def _build_limitations_tab(mode: str = "forza") -> tuple:
    ...
    if mode == "general":
        return (
            ...
            [
                (
                    "No in-game menu detection",
                    [
                        "Most games don't expose a clean \"in a menu\" signal to other apps, so Segue can't tell a pause menu from active play. It uses which window is focused: full volume while the game is focused, Unfocused Volume when you alt-tab out. The proper drive/menu auto-switch only exists in Forza mode (telemetry)."
                    ],
                ),
                (
                    "You'll steer it by hand sometimes",
                    [
                        "Apart from the focus + speech auto-adjust, Segue can't read your intent. You'll skip, pause and nudge volume yourself via the touchpad / in-app buttons."
                    ],
                ),
                (
                    "Speech ducking isn't perfect",
                    [
                        "Ducking listens to the picked game's audio and guesses when someone is talking. It can miss a line, duck a beat late, or dip briefly during loud effects. Tune Ducked Volume and Fade length to taste, or turn Speech recognition off in Mixer."
                    ],
                ),
                (
                    "Some games block audio capture",
                    [
                        "A few games (anti-cheat protected, exclusive audio) refuse process-loopback capture - ducking won't fire on those. Volume and skip still work fine."
                    ],
                ),
                (
                    "The overlay only shows in some captures",
                    [
                        "The now-playing overlay appears in display / monitor capture (OBS Display Capture, ShadowPlay desktop) - play borderless windowed. Pure game-capture and exclusive fullscreen won't show it, because Segue never draws inside the game."
                    ],
                ),
            ],
        )
    return (
        ...
        [
            (
                'The garage counts as "driving"',
                [
                    "Forza reports the garage, Forzavista and some menus as a live race, so Segue treats them as driving (full volume, D-pad can skip). That's why Lock Skip exists - turn it on when you're parked so songs don't jump."
                ],
            ),
            (
                "You'll steer it by hand sometimes",
                [
                    "Segue only knows driving vs menu vs speech - it can't read your intent. Sometimes you'll want to skip, pause or change volume yourself; that's what the controller and in-app buttons are for."
                ],
            ),
            (
                "Speech ducking isn't perfect",
                [
                    "Ducking listens to the game's audio and guesses when someone is talking. It can miss a line, duck a beat late, or dip briefly during loud effects. Tune Ducked Volume and Fade length to taste, or turn Speech recognition off if it bothers you."
                ],
            ),
            (
                "The overlay only shows in some captures",
...
def _build_bansafe_tab(mode: str = "forza") -> tuple:
    ...
    if mode == "general":
        return (
            ...
            [
                (
                    "What Segue touches - and doesn't",
                    [
                        "In Other-game mode, Segue uses Windows audio APIs only: it listens to the picked process's audio for speech detection (same API OBS / Discord use) and sets Spotify's per-app volume. No game files, no game memory, no code injection, no network calls. Nothing for anti-cheat to flag."
                    ],
                ),
                (
                    "The trade-off",
                    [
                        "Because Segue stays completely outside the game, it can't free its own buttons or draw inside the game for you. The touchpad belongs entirely to Segue and the D-pad belongs to the game - swipe to skip / change volume."
                    ],
                ),
                (
                    "Caveat",
                    [
                        "A few competitive games (anti-cheat protected, exclusive audio) block process-loopback capture. Segue still works for volume and skip, but ducking on speech won't fire."
                    ],
                ),
            ],
        )
    return (
        ...
        [
            (
                "What Segue touches - and doesn't",
                [
                    "Segue only reads Forza's official \"Data Out\" telemetry and sets Spotify's own volume. It never reads or writes game files, never touches game memory, never injects code, and never sends anything over the network. There is nothing for anti-cheat to flag."
                ],
            ),
            (
                "The trade-off",
                [
                    "Because it stays completely outside the game, Segue can't free its own buttons or draw inside the game for you. That's why you unbind the D-pad in Forza yourself (see Setup) and why the overlay needs display capture. A little manual setup is the price of staying safe - and worth it."
                ],
            ),
        ],
    )
        ...
        return (
            ...
            [
                (
                    "The volume sliders",
                    [
                        "Volume - your master level (the one your volume buttons / touchpad move). Segue scales Spotify's own volume, so it can't go louder than Spotify itself is set.",
                        "",
                        "Unfocused Volume - level when you alt-tab out of the game (0 = muted).",
                        "",
                        "Ducked Volume - level while Segue hears speech (only shown when Speech recognition is on).",
                    ],
                ),
                (
                    "Speech & fade",
                    [
                        "Speech recognition - turns ducking on or off. Off hides Ducked Volume + saves CPU.",
                        "",
                        "Fade length (Extras) - how long music takes to ramp back up after a duck. Longer is smoother but slower to recover.",
                    ],
                ),
            ],
        )
    return (
        ...
        [
            (
                "The three levels",
                [
                    "Volume - your master level. The same one your volume buttons move; applies instantly, in-game or not. Segue scales Spotify's own volume, so it can't go louder than Spotify itself is set.",
                    "",
                    "Menu Volume - level in menus and when paused (0 = muted).",
                    "",
                    "Ducked Volume - level while Segue hears speech in-game.",
                ],
            ),
            (
                "Speech & fade",
                [
                    "Speech recognition - turns ducking on or off. Off greys out Ducked Volume.",
                    "",
                    "Fade length (Extras) - how long music takes to ramp back up after a duck or a menu. Longer is smoother but slower to recover.",
                ],
            ),
        ],
    )
...
def _build_troubleshooting_tab(mode: str = "forza") -> tuple:
    ...
    if mode == "general":
        return (
            ...
            [
                (
                    'Game shows "not running"',
                    [
                        "Segue watches the exact process you picked. Launch the game first, then re-pick it via the game chevron if the name doesn't match. Auto-detect also switches when a known game (Forza, Rocket League) starts."
                    ],
                ),
                (
                    "Spotify not playing",
                    [
                        "Open Spotify and start a track. Segue follows whatever Spotify is playing - it doesn't start music on its own."
                    ],
                ),
                (
                    "Controller does nothing",
                    [
                        "Plug the controller in before launching, or wait a couple seconds for Segue to pick it up (it retries). DualSense + DualShock 4 work out of the box; other pads via Controls."
                    ],
                ),
                (
                    "Music too quiet everywhere",
                    [
                        "Raise the Volume slider AND check Spotify's own volume - Segue scales Spotify, it can't push past Spotify's own level."
                    ],
                ),
                (
                    "Music drops when I alt-tab",
                    [
                        "That's Unfocused Volume doing its job. Raise it (or set it to 100 %) in Mixer if you don't want the drop."
                    ],
                ),
            ],
        )
    return (
        ...
        [
            (
                "Forza not detected",
                [
                    "Enable Data Out on 127.0.0.1 port 5300 (Setup step 3).",
                    "",
            ...
            (
                "D-pad does nothing, or flips Forza's radio",
                [
                    "You haven't unbound D-pad Left / Right in Forza yet, or the in-game radio is still on. See Setup steps 1 and 2."
                ],
            ),
            (
                "Buttons dead while driving (Xbox / keyboard)",
                [
                    "Forza grabs those same inputs to drive. Free them inside Forza first, then rebind in Segue → Controls."
                ],
            ),
            (
                "Music too quiet everywhere",
                [
                    "Raise the Volume slider in Segue AND check Spotify's own volume - Segue scales Spotify, it can't push past Spotify's own level."
                ],
            ),
            (
                "Ducking too slow or jumpy",
                ["Raise Ducked Volume in Mixer, or tune Fade length in Extras."],
            ),
        ],
    )
...
def _build_setup_tab(device: str, mode: str = "forza", preset: str = "") -> tuple:
    ...
    _SPOTIFY_TIP = (
        "Tip",
        [
            "Crank Spotify to 80-100 %. Segue scales down from there - it can't go louder than Spotify's own slider."
        ],
        ["spotify_volume_tip.png"],
    )
    if preset == "rocketleague":
        rblocks = [
            (
                "Quick setup",
                [
                    "<b>1.</b>  In Rocket League → Settings → Audio, set Music Volume to 0 (so it doesn't clash with Spotify).<br><br><b>2.</b>  Plug in your DualSense or DualShock 4.<br><br><b>3.</b>  Launch Rocket League and play.<br><br>Touchpad swipe left / right skips tracks. Touchpad click (press) pauses and plays. Swipe up / down for volume."
                ],
            ),
            _SPOTIFY_TIP,
        ]
    ...
    if mode == "general":
        gblocks = [
            (
                "1.  Pick your game",
                [
                    'Click the chevron next to the game icon at the top → "Other game…" → pick from the list.',
                    "",
                    "Only apps currently making sound show up, so launch the game first.",
                ],
            ),
            (
                "2.  Mute the game's music",
                [
                    "In the game's own audio settings, set its music to 0 so it doesn't clash with Spotify (sound effects can stay up)."
                ],
            ),
            _SPOTIFY_TIP,
            (
                "3.  Play Spotify",
                ["Open Spotify and start a track. Segue controls its volume."],
            ),
            (
                "4.  Use the controls",
                [
                    "Touchpad swipe = skip / volume. Tap = pause. The D-pad belongs to the game by default - skip via touchpad instead.",
                    "",
                    "Speech ducking still works on the picked process - turn it off in Mixer if you don't want it.",
                ],
            ),
            (
                "Note",
                [
                    "General mode has no menu / paused detection (most games don't expose that). Music stays at Drive volume until you pause Spotify or change it yourself."
                ],
            ),
        ]
    ...
    if uses_dpad:
        radio_body = [
            "Forza → pause → Audio → Radio: Off.",
            "",
            "Until you do this, pressing D-pad Left / Right will skip Forza's radio stations instead of your Spotify tracks.",
        ]
    else:
        radio_body = [
            "Forza → pause → Audio → Radio: Off.",
            "",
            "Otherwise Forza's own radio plays on top of Spotify and they fight each other.",
        ]
    blocks.append(
        (f"{n}.  Turn the in-game radio OFF", radio_body, ["setup_radio.png"])
    )
    n += 1
    blocks.append(
        (
            f"{n}.  Turn the game's music OFF",
            [
                "Forza → pause → Audio → set Music to 0 (the menu / title-screen music).",
                "",
                "Otherwise it plays over Spotify whenever you're not out driving.",
            ],
            ["setup_music.png"],
        )
    )
    ...
    if uses_dpad:
        blocks.append(
            (
                f"{n}.  Unbind the D-pad in Forza",
                [
                    "Forza → Settings → Controls → unbind Radio Previous and Radio Next (D-pad Left / Right).",
                    "",
                    "Now Segue owns the D-pad and can skip Spotify tracks.",
                ],
                ["setup_controls.png"],
            )
        )
        ...
        if is_playstation:
            blocks.append(
                (
                    f"{n}.  Set up the LINK wheel (no skip clashes)",
                    [
                        "ANNA and Forza LINK are picked with the D-pad - the same buttons Segue skips with. So when you open a wheel, Segue locks skipping for 3 seconds: pick your phrase, then skip's back. ANNA opens on D-pad Down, so it's already covered.",
                        "",
                        "Forza LINK just needs two quick rebinds in Forza → Settings → Controls:",
                        "",
                        'a)  Set "Map" to Unbound. You won\'t miss it - the map is still one tap away via the Menu button (first option, already highlighted).',
                        ("img", "setup_link1.png"),
                        'b)  Bind "Forza LINK" to the View button - the Create / Share button, just left of the touchpad.',
                        ("img", "setup_link2.png"),
                        "",
                        "Done. Pressing View now opens LINK and arms the same skip-lock, so picking a phrase never skips a track.",
                    ],
                )
            )
            n += 1
    blocks.append(
        (
            f"{n}.  Turn on Data Out",
            [
                "Forza → Settings → HUD & Gameplay → Telemetry, then set:",
                "      Data Out:                 On",
                "      Data Out IP Address:  127.0.0.1",
                "      Data Out IP Port:        5300",
                "Same steps in Horizon 4 and 5 - Segue works with all of them.",
                "(Click an image below to see it full-size.)",
            ],
            ["setup_hud.png", "setup_telemetry.png"],
        )
    )
    n += 1
    blocks.append(
        (
            f"{n}.  Play Spotify",
            [
                "Open Spotify and start a track. Segue follows whatever Spotify is playing."
            ],
        )
    )
    n += 1
    ...
    blocks.append(
        (
            f"{n}.  Launch Forza",
            [
                'Tick "Auto-start" in Extras to launch Segue with the game, or just run it yourself.',
                "",
                "The top bar turns green when both Forza and Spotify are detected.",
            ],
        )
    )
    blocks.append(
        (
            "Playing on Xbox? (experimental)",
            [
                "Forza on the console, music on this PC - Segue can follow the game over your home network:",
                "",
                '1)  Find this PC\'s local IP: press Win+R, run "cmd", type "ipconfig" - grab the IPv4 address (looks like 192.168.x.x).',
                "2)  In Forza ON THE XBOX: Settings → HUD & Gameplay → Telemetry → Data Out: On, IP Address = your PC's IP, Port 5300.",
                "3)  Run Segue here with the Forza preset, music on this PC. If Windows asks about the firewall, allow on Private networks.",
                "",
                'Menus and races now drive your music volume just like on PC. Controller buttons stay PC-only (your pad talks to the console) - keyboard binds and Ctrl+Shift+Alt+S still work. The status bar may say "Forza not detected"; that\'s cosmetic, telemetry still flows.',
                "",
                'Tip: listen to the console through the Xbox app (Remote Play) on this PC and turn on "Include Discord" - speech ducking then works on the game audio too.',
                "",
                "No data arriving? Forza's console build drops Data Out sometimes. The fix that works: re-enter the IP and port in Forza (even if they look right), then FULLY quit the game - highlight it on the dashboard, Menu button, Quit. Quick Resume keeps dead connections alive, so the Home button isn't enough. Relaunch and drive.",
            ],
        )
    )
    ql = _setup_quick_look(device, mode)
...
def _build_controls_tab(device: str, mode: str = "forza", preset: str = "") -> tuple:
    ...
    blocks.append(
        (
            "Pick a device",
            [
                "Controls → Select Device. PlayStation (Best) gives full touchpad control out of the box; Xbox + keyboard (Beta) and sim wheel (Beta) are fully bindable. Switching device applies right away - no restart needed."
            ],
        )
    )
    if device == "playstation":
        if mode == "general":
            ...
            pause_caption = (
                "Touchpad press → pause / play"
                if pause_is_press
                else "Tap → pause / play"
            )
            ps_clips = [
                ("Swipe right → next track", "clips/touchpad_swipe_right.gif", 150),
                ("Swipe left → previous track", "clips/touchpad_swipe_left.gif", 150),
                ("Swipe up / down → volume", "clips/touchpad_swipe_up.gif", 150),
                (pause_caption, "clips/touchpad_tap.gif", 150),
            ]
            ...
            blocks.append(
                (
                    "Also",
                    [
                        "•  Skip can also be the left-stick click (L3) - switch it in Controls → Skip with.",
                        "•  Mute button  →  Lock Skip.",
                    ],
                )
            )
        else:
            ps_clips = [
                (
                    ...
                    "clips/dpad_press_right_left.gif",
                    150,
                ),
                ("Swipe up / down → volume", "clips/touchpad_swipe_up.gif", 150),
                ("Tap → pause / play", "clips/touchpad_tap.gif", 150),
            ]
            blocks.append(("PlayStation defaults", [], [], ps_clips, 2))
            blocks.append(
                (
                    ...
                    [
                        "•  Mute button  →  Lock Skip",
                        "•  Skipping only fires while you're actually in a race, so it can't trigger by accident in menus.",
                    ],
                )
            )
    elif device == "xbox":
        blocks.append(
            (
                ...
                [
                    "•  D-pad Left / Right  →  skip track",
                    "",
                    "Everything else (volume, pause) is bindable in Controls → Rebind.",
                ],
                [],
                [
                    (
        ...
        blocks.append(
            (
                ...
                [
                    ...
                    "•  ;      →  Lock Skip",
                    "",
                    "Rebind in Controls → Rebind if any of these collide with another app.",
                ],
            )
        )
    else:
        blocks.append(
            (
                "Sim wheel",
                [
                    "All buttons are bindable in Controls → Rebind. No fixed scheme since wheels vary wildly. If your wheel has a D-pad on the hub (Logitech G29, Thrustmaster T300, etc.), the example below is what skipping looks like."
                ],
                [],
                [
                    (
    ...
    blocks.append(
        (
            "Lock Skip & Turn Off",
            [
                "Lock Skip - freezes track-skipping. Toggle it on the controller or in the app. See Limitations for why the garage needs it.",
                "",
                "Turn Off - hands Spotify back to you and stops Segue touching the volume, without closing the app. Turn On to resume.",
            ],
        )
    )
...
_HELP_TABS = [
    (
        ...
        [
            ...
            (
                "4.  Set up the LINK wheel (no skip clashes)",
                [
                    "ANNA and Forza LINK are picked with the D-pad - the same buttons Segue skips with. So when you open a wheel, Segue locks skipping for 3 seconds: pick your phrase, then skip's back. ANNA opens on D-pad Down, so it's already covered.",
                    "",
                    "Forza LINK just needs two quick rebinds in Forza → Settings → Controls:",
        ...
        [
            (
                ...
                [
                    "Volume - your master level. The same one your volume buttons move; applies instantly, in-game or not. Segue scales Spotify's own volume, so it can't go louder than Spotify itself is set.",
                    "",
                    "Menu Volume - level in menus and when paused (0 = muted, the default).",
                    "",
                    "Ducked Volume - level while Segue hears speech in-game.",
                ],
            ),
    ...
    (
        ...
        [
            (
                "Pause it",
                [
                    "Hit Turn Off to hand Spotify's volume back to you. Segue stays open but stops touching the volume; Turn On to resume."
                ],
            ),
            (
                "Stop it auto-starting",
                [
                    'Untick "Auto-start" in Extras so it no longer launches with your games.'
                ],
            ),
            (
                "Quit",
                [
                    "☰ → Quit Segue, or close the window. Spotify's volume is restored on exit."
                ],
            ),
            (
                "Uninstall",
                [
                    "Use ☰ → Uninstall Segue, or Windows Settings → Apps → Segue → Uninstall. The uninstaller stops Segue cleanly, removes the app, the autostart shortcut, and your settings in %APPDATA%\\Segue. Nothing left behind."
                ],
            ),
        ],
    ),
...
class _HelpWindow(QWidget):
    ...
    def __init__(
        ...
        self._curated_preset = (
            preset if preset in ("forza", "rocketleague") else "forza"
        )
        self._setup_view_mode = (
            self._curated_preset
            if mode != "general" or preset == "rocketleague"
            else "general"
        )
        ...
        for tabname, blocks in self._tabs:
            ...
            if tabname == "Controls":
                ...
                cur_label = self._DEV_LABEL.get(
                    self._setup_device,
                    self._setup_device.title() if self._setup_device else "?",
                )
                dev_lbl = QLabel(
                    f"Current device: <b style='color:{_ACCENT};'>{cur_label}</b>"
                )
            ...
            if tabname == "Setup":
                ...
                self._setup_forza_btn.clicked.connect(
                    lambda: self._switch_setup_view(self._curated_preset)
                )
                self._setup_other_btn.clicked.connect(
                    lambda: self._switch_setup_view("general")
                )
            ...
            for i, blk in enumerate(blocks):
                ...
                for img_name in imgs:
                    ...
                    img_lbl.setStyleSheet(
                        f"border: 1px solid {_c('border')}; border-radius: {_s(4)}px;"
                    )
                    ...
                    img_lbl.mousePressEvent = lambda _e, path=p_img: (
                        self._open_image_external(path)
                    )
                    v.addSpacing(_s(4))
                    v.addWidget(img_lbl)
    ...
    def _update_setup_toggle_state(self) -> None:
        ...
        for btn, on in (
            (self._setup_forza_btn, curated),
            (self._setup_other_btn, not curated),
        ):
...
class _KeycapButton(QPushButton):
    ...
    def __init__(self, text, parent=None):
        ...
        self.setStyleSheet(
            f"QPushButton#keycapbtn {{ color:{_c('text')}; background:{_c('deep')}; border:2px solid {_c('border_hi')}; border-radius:{_s(8)}px; padding:{_s(6)}px {_s(16)}px; }} QPushButton#keycapbtn:hover {{ border-color:{_ACCENT}; background:{_c('accent_tint')}; }}"
        )
...
class _CaptureDialog(QDialog):
    ...
    def __init__(self, reader, parent=None, kind="button", owner_of=None, device=None):
        ...
        self._device = device
        self._prompt_text = (
            "Press a key, or a combo like Ctrl+Shift+P…"
            if kind == "key"
            else "Press a button, or hold two together for a combo…"
        )
        ...
        self._again.setMinimumWidth(
            self._again.fontMetrics().horizontalAdvance("Press again") + _s(28)
        )
        ...
        if device in ("playstation", "dualsense", "dualshock"):
            ...
            self._again.setMinimumWidth(
                self._again.fontMetrics().horizontalAdvance("Press again  (B)") + _s(28)
            )
        ...
        try:
            self._baseline = {
                p for p in (self._reader.read_pressed() or "").split("+") if p
            }
        except Exception:
            self._baseline = set()
        self._timer.start()
    ...
    def _arm_confirm(self, on):
        ...
        if on:
            self._confirm.setStyleSheet(
                f"QPushButton {{ background:{_c('btn_fill')}; color:{_c('btn_text')}; border:2px solid {_ACCENT}; border-radius:{_s(8)}px; padding:{_s(7)}px {_s(10)}px; font-weight:700; }}QPushButton:hover {{ background:{_c('btn_fill_hi')}; }}"
            )
            return None
        self._confirm.setStyleSheet("")
        return None
...
class _RebindDialog(QDialog):
    ...
    _DS_FIXED = [
        "Skip track - D-pad Left / Right",
        "Volume - touchpad swipe up / down",
        "Pause - touchpad tap",
    ]
    ...
    def __init__(self, cfg, on_save, on_restart=None, ui=None, parent=None):
        ...
        self._wds = {
            "safe_mode_button": cfg.safe_mode_button,
            "pause_button": cfg.pause_button,
            "skip_button": cfg.skip_button,
            "open_button": getattr(cfg, "open_button", "micBtn"),
            "latch_button": getattr(cfg, "latch_button", "share"),
        }
        self._wbind_by_dev = {
            d: dict(b)
            for d, b in (getattr(cfg, "bindings_by_device", {}) or {}).items()
        }
        self._whold_by_dev = {
            d: set(h)
            for d, h in (getattr(cfg, "hold_actions_by_device", {}) or {}).items()
        }
        ...
        if (
            cfg.skip_button
            and (not cfg.gamepad_skip_enabled)
            and (not cfg.touchpad_skip_enabled)
        ):
        ...
        self._orig_pause = (
            cfg.pause_input if cfg.pause_input in ("tap", "press") else "tap"
        )
        self._wpause = self._orig_pause
        self._orig_tapsens = int(getattr(cfg, "tap_sensitivity", 70))
        self._wtapsens = self._orig_tapsens
        self._orig_volsens = int(
            round(float(getattr(cfg, "vol_hold_sensitivity", 1.0)) * 100)
        )
        ...
        for dev, label in [(d, l) for d, l in self._DEVICES if d in _allowed]:
            ...
            pm = _dev_pixmap(dev, _ic) or _load_scaled(
                os.path.join(_ASSETS, dev + ".png"), _ic
            )
    ...
    def _has_custom_binds(self) -> bool:
        ...
        fresh = Config()
        return (
            self._wds.get("safe_mode_button") != fresh.safe_mode_button
            or self._wds.get("pause_button") != fresh.pause_button
            or self._wds.get("skip_button") != fresh.skip_button
            or self._wds.get("open_button") != fresh.open_button
        )
    ...
    def _set_device(self, dev):
        ...
        if not self._opentrig_touched and getattr(self._cfg, "open_trigger", "") in (
            "hold",
            "press",
        ):
    ...
    def _rebuild_content(self):
        ...
        if dev in ("playstation", "dualsense", "dualshock"):
            ...
            _sw_row.addWidget(
                _InfoPopupLabel(self._fixed_controls_clips), 0, Qt.AlignVCenter
            )
            ...
            if is_forza_preset:
                self._cl.addLayout(
                    self._ds_row("Pause / play", "pause_button", "Touchpad tap", "")
                )
            else:
                _tp_row = QHBoxLayout()
                ...
                _tp_row.addWidget(
                    _InfoPopupLabel(lambda: self._fixed_controls_clips("pause")),
                    0,
                    Qt.AlignVCenter,
                )
            ...
            if (
                ...
                _tsl.valueChanged.connect(
                    lambda x, lbl=_tval: (
                        lbl.setText("Off" if x <= 0 else f"{x}%"),
                        self._on_tap_sensitivity(x),
                    )
                )
                _tv.addWidget(_tsl)
            ...
            if is_forza_preset:
                self._cl.addLayout(
                    self._ds_row(
                        "Interact button", "latch_button", "Square", self._INTERACT_TIP
                    )
                )

            def _ps_more(v):
                v.addLayout(
                    self._ds_row("Lock Skip", "safe_mode_button", "", self._LOCK_TIP)
                )
                v.addLayout(
                    self._ds_row(
                        "Open Segue", "open_button", "", self._OPEN_TIP, suffix="(hold)"
                    )
                )
            ...
            if dev in ("xbox", "wheel"):
                ...
                _vhsl.valueChanged.connect(
                    lambda x, lbl=_vhval: (
                        lbl.setText(f"{x}%"),
                        self._on_vol_hold_sensitivity(x),
                    )
                )
            ...
            if dev == "wheel":
                self._cl.addWidget(
                    self._info_line(
                        "Sim wheel is untested. I don't own one, so let's just pray it works 🙏"
                    )
                )
            if dev == "keyboard":
                self._cl.addWidget(
                    self._info_line(
                        "Safe defaults that don't clash with most games. Click any key-cap to rebind it."
                    )
                )
    ...
    def _pause_input_row(self, trailing_stretch=True):
        ...
        for btn, key in (
            (self._pause_tap_btn, "tap"),
            (self._pause_press_btn, "press"),
        ):
            btn.setObjectName("skipbtn")
            btn.setCheckable(True)
            btn.setCursor(Qt.PointingHandCursor)
    ...
    def _do_hotkey(self, cap):
        ...
        try:
            dlg = _CaptureDialog(
                None, self, kind="key", owner_of=None, device="keyboard"
            )
    ...
    def _open_trigger_row(self):
        ...
        for btn, key in (
            (self._opentrig_hold_btn, "hold"),
            (self._opentrig_press_btn, "press"),
        ):
            btn.setObjectName("skipbtn")
            btn.setCheckable(True)
            btn.setCursor(Qt.PointingHandCursor)
    ...
    def _fixed_controls_clips(self, kind: str = "skip") -> QWidget:
        ...
        pause_clip = (
            "clips/touchpad_press.gif"
            if self._wpause == "press"
            else "clips/touchpad_tap.gif"
        )
        if not os.path.exists(os.path.join(_ASSETS, pause_clip)):
            pause_clip = "clips/touchpad_tap.gif"
        if kind == "pause":
            clips = [
                ("Volume", "Swipe up / down", "clips/touchpad_swipe_up.gif"),
                ("Pause / Play", pause_sub, pause_clip),
            ]
            target_w = _s(115)
        elif self._wskip == "touchpad":
            clips = [
                ("Next", "Swipe right", "clips/touchpad_swipe_right.gif"),
                ("Previous", "Swipe left", "clips/touchpad_swipe_left.gif"),
            ]
            target_w = _s(115)
    ...
    def _ds_row(self, label, attr, fallback, tip, suffix=""):
        ...
        cap.setToolTip("Click to rebind")
        cap.clicked.connect(
            lambda _=False, a=attr, c=cap, f=fallback, sx=suffix: self._do_ds(
                a, c, f, suffix=sx
            )
        )
        row.addStretch(1)
    ...
    def _action_row(self, action):
        ...
        eff = _ib.effective_bindings(
            self._wdev, self._wbind, getattr(self._cfg, "mode", None)
        )
        cap = _KeycapButton("")
    ...
    def _style_trigpill(self, pill, action):
        ...
        pill.setStyleSheet(
            f"QPushButton#trigpill {{ color: {_c('text') if on else _c('text_dim')}; border: 1px solid {_ACCENT if on else _c('border')}; border-radius: {_s(6)}px; padding: {_s(2)}px {_s(9)}px; font-weight: {'700' if on else '500'}; background: {'rgba(167,159,138,0.16)' if on else 'transparent'}; }}QPushButton#trigpill:hover {{ border-color: {_ACCENT}; }}"
        )
        return None
    ...
    _SHARE_OK = {
        ("open_button", "safe_mode_button"),
        ("safe_mode_button", "open_button"),
    }
    ...
    def _bind_owner(self, code, skip_action=None, skip_attr=None):
        ...
        for a, c in eff.items():
            if c != code or a == skip_action:
                continue
            if (
                self._wdev in ("xbox", "wheel")
                and skip_action in self._HOLDABLE
                and a in self._HOLDABLE
            ):
                continue
    ...
    def _capture_run(self, owner_of=None):
        ...
        if self._wdev == "keyboard":
            dlg = _CaptureDialog(
                None, self, kind="key", owner_of=owner_of, device=self._wdev
            )
        ...
        if self._wdev != self._orig_device:
            QMessageBox.information(
                self,
                "Save first",
                "Save the device switch first, then rebind its buttons.",
            )
            return None
        ...
        if reader is None or not hasattr(reader, "read_pressed"):
            ...
            return None
        dlg = _CaptureDialog(
            reader, self, kind="button", owner_of=owner_of, device=self._wdev
        )
        if dlg.exec() == QDialog.Accepted and dlg.code is not None:
            return dlg.code
        return None
    ...
    def _reset_binds(self):
        ...
        self._whold = set()
        self._wds = {
            "safe_mode_button": fresh.safe_mode_button,
            "pause_button": fresh.pause_button,
            "skip_button": fresh.skip_button,
            "open_button": fresh.open_button,
            "latch_button": fresh.latch_button,
        }
        self._rebuild_content()
        self._update_save_btn()
        return None
    ...
    def _binds_dirty(self) -> bool:
        return (
            self._wds["safe_mode_button"] != self._cfg.safe_mode_button
            or self._wds["pause_button"] != self._cfg.pause_button
            or self._wds["open_button"] != getattr(self._cfg, "open_button", "micBtn")
            or self._wds["latch_button"] != getattr(self._cfg, "latch_button", "share")
            or self._cur_bind_map()
            != {d: b for d, b in self._orig_bind_by_dev.items() if b}
            or self._cur_hold_map()
            != {d: h for d, h in self._orig_hold_by_dev.items() if h}
            or self._wskip != self._orig_skip
            or self._wpause != self._orig_pause
            or self._wopentrig != self._orig_opentrig
            or self._whotkey != self._orig_hotkey
            or self._wtapsens != self._orig_tapsens
            or self._wvolsens != self._orig_volsens
        )
    ...
    def _update_save_btn(self):
        ...
        self._save_btn.setEnabled(
            self._wdev != self._orig_device or self._binds_dirty()
        )
        if getattr(self, "_reset_btn", None) is not None:
            self._reset_btn.setVisible(self._has_custom_binds())
            return None
    ...
    def _commit(self):
        ...
        if self._opentrig_touched or getattr(self._cfg, "open_trigger", "") in (
            "hold",
            "press",
        ):
        ...
        self._wbind_by_dev[self._wdev] = dict(self._wbind)
        self._whold_by_dev[self._wdev] = set(self._whold)
        self._cfg.bindings_by_device = {
            d: dict(b) for d, b in self._wbind_by_dev.items() if b
        }
        self._cfg.hold_actions_by_device = {
            d: sorted(h) for d, h in self._whold_by_dev.items() if h
        }
    ...
    def exec(self):
        if sys.platform == "win32":
            try:
                ...
                _dark = ctypes.byref(
                    ctypes.c_int(0 if _active_theme() == "light" else 1)
                )
    ...
    def _uncloak(self):
        ...
        try:
            ctypes.windll.dwmapi.DwmSetWindowAttribute(
                int(self.winId()), 13, ctypes.byref(ctypes.c_int(0)), 4
            )
        except Exception:
            pass
        return None
...
class _CustomSourceDialog(QDialog):
    ...
    def _populate(self, apps):
        ...
        if not apps:
            self._msg.setText(
                "Nothing's playing yet. Start an app that isn't already a source and it'll show up here."
            )
            QTimer.singleShot(0, self.adjustSize)
            return None
        self._msg.setText(
            "These apps are playing right now. Pick the one with your music."
        )
        for app in apps:
            ...
            btn.setCursor(Qt.PointingHandCursor)
            btn.setIcon(
                _exe_icon(getattr(app, "path", ""), (label[:1] or "?").upper(), 22)
            )
...
def _style_expander(btn, is_open=None):
    ...
    btn.setStyleSheet(
        f"QPushButton {{ background: {_c('sunk')}; border: 1px solid {_c('border')}; border-radius: {_s(7)}px; color: {_c('text_dim')}; padding: {_s(6)}px {_s(10)}px; }}QPushButton:hover {{ background: {_c('surface_hi')}; color: {_c('text')}; border-color: {_c('border_hi')}; }}"
    )
    if is_open is not None:
        ...
        def _spin_chev(b=btn):
            ...
            an.valueChanged.connect(
                lambda v, bb=b: (setattr(bb, "_chev_angle", float(v)), bb.update())
            )
            an.start()
            b._chev_anim = an
            return None
...
class _IconPopup(QFrame):
    ...
    def __init__(
        ...
        self.setStyleSheet(
            f"QPushButton#popupitem {{ background: transparent; border: none; color: {_c('text')}; text-align: left; padding: {_s(8)}px {_s(12)}px; border-radius: {_s(6)}px; }}QPushButton#popupitem:hover {{ background: {_c('surface_hi')}; }}"
        )
    ...
    def _build(self, items, sub: bool):
        ...
        for key, label, pm in items:
            self._row(
                label,
                pm,
                lambda _=False, k=key: self._on_pick(k),
                checked=key == self._cur,
            )
        if not sub:
            if self._more_items:
                self._more_btn = self._row(
                    self._more_label + "  ›",
                    _dots_pixmap(22),
                    lambda _=False: self._swap(True),
                )
    ...
    def eventFilter(self, obj, ev):
        if ev.type() == QEvent.MouseButtonPress and (
            not self.frameGeometry().contains(ev.globalPosition().toPoint())
        ):
            self.close()
    ...
    def mousePressEvent(self, e):
        panel = self.rect().adjusted(
            self._shadow_pad, self._shadow_pad, -self._shadow_pad, -self._shadow_pad
        )
...
class _HoverLink(QLabel):
    ...
    def __init__(
        self, text: str, url: str, icon_path: str = "", size: int = 13, parent=None
    ):
        super().__init__(parent)
        self._t = text
        self._url = url
    ...
    def _render(self, hover: bool):
        ...
        sz = _s(16)
        img = (
            f"&nbsp;<img src='{self._icon}' width='{sz}' height='{sz}' style='vertical-align:middle;'>"
            if self._icon
            else ""
        )
        self.setText(
            f"<span style='color:{_c('text')}; text-decoration:{deco}; font-weight:bold;'>{self._t}</span>{img}"
        )
        return None
...
class _PresetsPopup(QFrame):
    ...
    def __init__(self, parent, names: list):
        super().__init__(
            parent, Qt.Popup | Qt.FramelessWindowHint | Qt.NoDropShadowWindowHint
        )
        self.setAttribute(Qt.WA_DeleteOnClose)
    ...
    def _build(self, names: list) -> None:
        ...
        if names:
            ...
            sep.setStyleSheet(
                f"background: {_c('border')}; margin: {_s(4)}px {_s(4)}px;"
            )
...
class _DevicePickerDialog(QDialog):
    ...
    _ALL_DEVS = [
        ("playstation", "PlayStation", "Best"),
        ("xbox", "Xbox", "Beta"),
        ("keyboard", "Keyboard", ""),
        ("wheel", "Sim wheel", "Beta"),
    ]
    ...
    def __init__(self, cfg, parent=None):
        ...
        for i, (key, name, tag) in enumerate(devs):
            icon = (
                _dev_qicon(key, 46)
                or _load_icon(os.path.join(_ASSETS, key + ".png"), 46)
                or (
                    _load_icon(os.path.join(_ASSETS, "dualsense.png"), 46)
                    if key == "playstation"
                    else None
                )
                or _dev_icon(key, 46)
...
class _GamePresetPickerDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        v.addWidget(title)
        sub = QLabel(
            "Segue tunes itself for the game you pick.  You can switch anytime."
        )
        sub.setObjectName("hint")
        sub.setFont(_ui_font(14))
        ...
        for key, preset in _gp.GAME_PRESETS.items():
            ...
            if key == "forza":
                ...
                if os.path.exists(cand):
                    icon_path = cand
            icon = (_load_icon(icon_path, 46) if icon_path else None) or _dev_icon(
                "xbox", 46
            )
...
class _GamePickerDialog(QDialog):
    ...
    def __init__(self, parent, current: str = ""):
        ...
        v.addWidget(t)
        sub = QLabel(
            "Only apps currently making sound show up. Launch the app first, then pick it here."
        )
        sub.setObjectName("hint")
        sub.setWordWrap(True)
        v.addWidget(sub)
        ...
        for s in sessions:
            try:
                ...
                if not pretty:
                    ...
                    pretty = (
                        stem_name[:1].upper() + stem_name[1:] if stem_name else name
                    )
...
class _OverlayPositionPicker(QWidget):
    ...
    def _refresh_tooltip(self) -> None:
        if self._free_mode:
            self.setToolTip(
                "Drag the overlay anywhere. Double-click to snap back to the nearest preset (right-click also resets)."
            )
            return None
        self.setToolTip(
            "Click a spot to snap to a preset, or drag the pill anywhere for free placement. Double-click / right-click resets."
        )
        return None
...
def _make_about_page():
    ...
    pv.addWidget(t)
    body = QLabel(
        "I'm a solo developer and I built Segue myself.<br><br>It's a work in progress, so bugs can happen.<br><br>Got a bug or an idea? Join the Discord and I'll read it."
    )
    ...
    pv.addLayout(brow)
    kofi = _HoverLink(
        "or support development on Ko-fi",
        _SUPPORT_URL,
        icon_path=os.path.join(_ASSETS, "kofi_logo.png"),
        size=13,
    )
...
class _WelcomeDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        v.addWidget(t)
        body = QLabel(
            f"Quick guided tour of what each part of the window does. Takes ~20 seconds. <span style='color:{_ACCENT}; font-weight:bold;'>(recommended)</span>"
        )
        body.setObjectName("hint")
        body.setFont(_ui_font(14))
        body.setWordWrap(True)
...
class _TourCallout(QFrame):
    ...
    def __init__(self, parent, on_next, on_skip):
        ...
        self.setStyleSheet(
            f"QFrame#tourcallout {{ background: {_c('panel')}; border: 2px solid {_ACCENT}; border-radius: {_s(10)}px; }}"
        )
        v = QVBoxLayout(self)
...
class _StreamOverlayEditor(QDialog):
    ...
    def __init__(self, cfg, parent=None):
        ...
        v.addLayout(row)
        hint = QLabel(
            "In OBS: Sources → + → Browser Source → paste this. It's transparent; size + position it in OBS."
        )
        ...
        ehint = QLabel(
            "Opens in your browser: drag the handle to resize, set background, colors, cover shape, show/hide. Saves + goes live instantly."
        )
...
class _ForzaSetupGate(QDialog):
    ...
    def __init__(self, device: str = "playstation", parent=None):
        ...
        self._stack.addWidget(
            page(
                ...
                [
                    ...
                    (
                        "text",
                        "Forza → <b>Settings</b> → <b>HUD &amp; Gameplay</b> → <b>Telemetry</b>, then set:",
                    ),
                    (
                        "text",
                        "Data Out: <b>On</b><br>IP Address: <b>127.0.0.1</b><br>Port: <b>5300</b>",
                    ),
        ...
        if uses_dpad and is_ps:
            self._stack.addWidget(
                page(
                    "Free the D-pad for skipping",
                    [
                        (
                            "text",
                            "Segue skips with <b>D-pad Left / Right</b>, so free them up:",
                        ),
                        (
                            "text",
                            "<b>1.</b> Unbind <b>Radio Prev / Next</b> AND <b>Telemetry Prev / Next</b>.",
                        ),
                        ("img", "setup_controls.png"),
                        (
                            "text",
                            "<b>2.</b> Move <b>Forza LINK</b> off the D-pad onto the <b>View button</b> (left of the touchpad).",
                        ),
                        ("img", "setup_link2.png"),
                    ],
                )
            ...
            self._stack.addWidget(
                page(
                    ...
                    [
                        ...
                        (
                            "text",
                            "Unbind <b>Radio Prev / Next</b> AND <b>Telemetry Prev / Next</b>.",
                        ),
                        ("img", "setup_controls.png"),
                    ],
                )
    ...
    def _ctrl_keycaps(self, keys):
        ...
        for k in keys:
            ...
            lbl.setStyleSheet(
                f"color:{_c('text')}; background:{_c('deep')}; border:2px solid {_c('border_hi')}; border-radius:{_s(8)}px; padding:{_s(8)}px {_s(14)}px;"
            )
            h.addWidget(lbl)
        h.addStretch(1)
        return cont
    ...
    def _ctrl_gif(self, clip, w=150):
        ...
        tw = _s(w)
        th = (
            int(tw * src.height() / src.width())
            if src.isValid() and src.width()
            else _s(90)
        )
        lbl = _RoundedMovieLabel(_s(6))
        mv = QMovie(p)
    ...
    def _controls_page(self, device):
        ...
        if device in ("playstation", "dualsense", "dualshock"):
            pv.addWidget(
                self._ctrl_row(
                    self._ctrl_gif("clips/touchpad_swipe_up.gif"),
                    "Volume",
                    "Swipe up / down on the touchpad",
                )
            )
            pv.addWidget(
                self._ctrl_row(
                    self._ctrl_gif("clips/dpad_press_right_left.gif"),
                    "Skip track",
                    "D-pad Left / Right",
                )
            )
            pv.addWidget(
                self._ctrl_row(
                    self._ctrl_gif("clips/touchpad_tap.gif"),
                    "Pause / play",
                    "Tap the touchpad",
                )
            )
            ...
            pv.addWidget(
                self._ctrl_row(
                    self._ctrl_keycaps(["=", "-"]), "Volume", "= louder, - quieter"
                )
            )
            pv.addWidget(
                self._ctrl_row(
                    self._ctrl_keycaps(["]", "["]), "Skip track", "] next, [ previous"
                )
            )
            pv.addWidget(
                self._ctrl_row(
                    self._ctrl_keycaps(["\\"]), "Pause / play", "Backslash key"
                )
            )
        elif device == "xbox":
            pv.addWidget(
            ...
            pv.addWidget(
                self._ctrl_row(
                    QLabel(),
                    "Volume / Pause",
                    "Pick free buttons in Controls after setup",
                )
            )
        else:
            pv.addWidget(
                self._ctrl_row(
                    QLabel(), "All controls", "Bind them in Controls after setup"
                )
            )
...
class _GeneralIntroGate(QDialog):
    ...
    def __init__(
        self,
        device: str = "playstation",
        game_label: str = "Rocket League",
        parent=None,
    ):
        super().__init__(parent)
        self.setWindowTitle(
            f"Segue - {game_label}" if game_label else "Segue - Controls"
        )
        if os.path.exists(_APP_ICON):
            self.setWindowIcon(QIcon(_APP_ICON))
        self.setStyleSheet(_build_qss(_CHECK))
        ...
        if is_kb:
            ...
            _hic = (
                _dev_qicon("playstation", 24)
                or _load_icon(os.path.join(_ASSETS, "dualsense.png"), 24)
                or _dev_icon("playstation", 24)
            )
        ...
        lead = (
            f"{game_label} is plug & play, no setup."
            if game_label
            else "Plug and play, no setup."
        )
        ...
        def keycap(text):
            ...
            k.setStyleSheet(
                f"color:{_c('text')}; background:{_c('deep')}; border:2px solid {_c('border_hi')}; border-radius:{_s(8)}px; padding:{_s(10)}px {_s(18)}px;"
            )
            return k
        ...
        rebind_note = (
            "Rebindable",
            f"Change any of these in Controls. Tip: set {music_owner}'s music to 0 so it doesn't clash with Spotify.",
        )
        self._stack = QStackedWidget()
        if is_kb:
            self._stack.addWidget(
            ...
            self._stack.addWidget(
                page(
                    "Skip / previous track",
                    "clips/touchpad_swipe_right.gif",
                    ["Swipe right = next track, left = previous."],
                )
            )
            self._stack.addWidget(
                page(
                    "Pause / play",
                    "clips/touchpad_tap.gif",
                    ["Tap the touchpad (or set it to Press in Controls)."],
                    note=rebind_note,
                )
            )
        else:
            self._stack.addWidget(
                page(
                    ...
                    "",
                    [
                        "Segue has no preset buttons for your controller here.",
                        "",
                        "Open Controls to bind Skip, Volume and Pause to buttons you don't use in-game.",
                        "",
                        f"Tip: set {music_owner}'s music to 0 so it doesn't clash with Spotify.",
                    ],
                )
            )
        self._stack.addWidget(_make_about_page())
...
class _TourOverlay(QWidget):
    ...
    def __init__(
        ...
        on_done,
        on_finish_cta=None,
        final_cta_label: str = "Done",
        final_secondary_label: str = "Skip",
    ):
        super().__init__(
            None,
    ...
    def _apply_step(self):
        ...
        self._callout.set_step(
            ...
            secondary_label=self._final_secondary,
        )
        self._position_callout()
        self.update()
        return None
...
class _PreferencesDialog(QDialog):
    ...
    def _build_spotify_connect_row(self, parent_layout):
        ...
        if not _sp_pm.isNull():
            _sp_logo.setPixmap(
                _sp_pm.scaled(
                    _s(28), _s(28), Qt.KeepAspectRatio, Qt.SmoothTransformation
                )
            )
        ...
        _sp_info = _InfoLabel(
            "Lets the overlay scroll through your Spotify queue (previous and upcoming tracks) and skip from there. Premium required for skip control."
        )
        ...
        if h.get("auth"):
            if h.get("premium") is False:
                self._sp_status.setText(
                    "●  Connected<span style='color:%s;font-size:11px;'>  ·  Premium required for skip</span>"
                    % _c("text_dim")
                )
    ...
    def _build_startup_row(self, parent_layout):
        ...
        self._startup_cb.setToolTip(
            "Launch Segue automatically when Windows starts (runs in the tray, no game needed)."
        )
        try:
        ...
        self._autostart_cb.setToolTip(
            "Launch Segue automatically when any of your games starts (Forza, Rocket League, or your picked game)."
        )
        try:
        ...
        if _KB_SUMMON:
            ...
            self._kbsummon_cb.setToolTip(
                "Hold CapsLock to bring up the music controls (a keyboard alternative to a mouse side button). A normal tap still toggles Caps Lock."
            )
            try:
                self._kbsummon_cb.setChecked(
                    bool(getattr(self._cfg, "keyboard_summon", False))
                )
            except Exception:
                pass
    ...
    def _sync_startup_checks(self):
        try:
            ...
            for cb, want in (
                (getattr(self, "_startup_cb", None), "direct"),
                (getattr(self, "_autostart_cb", None), "watch"),
            ):
                if cb is None:
                    continue
...
class SettingsWindow(QWidget):
    ...
    def __init__(
        ...
        self._caret_pm = _tinted(
            os.path.join(_ASSETS, "down-chevron.png"), _caret_col, 12
        )
        ...
        self._forza_lbl.setToolTip(
            "Game - click to open it (launch or bring to front); use the chevron to switch game"
        )
        ...
        self._forza_lbl.enterEvent = lambda e, b=self._forza_lbl: (
            QPushButton.enterEvent(b, e),
            setattr(b, "_launch_glow", 1.0),
            self._apply_game_icon(),
            self._set_game_side("icon"),
            b.update(),
        )
        self._forza_lbl.leaveEvent = lambda e, b=self._forza_lbl: (
            QPushButton.leaveEvent(b, e),
            setattr(b, "_launch_glow", 0.4),
            self._apply_game_icon(),
            self._game_side_recheck(),
            b.update(),
        )
        ...
        self._game_caret.paintEvent = lambda e, b=self._game_caret: (
            self._paint_caret_btn(b, e, "_game_caret_angle", "_game_hover_side")
        )
        self._game_caret.enterEvent = lambda e, b=self._game_caret: (
            QPushButton.enterEvent(b, e),
            self._set_game_side("caret"),
            b.update(),
        )
        self._game_caret.leaveEvent = lambda e, b=self._game_caret: (
            QPushButton.leaveEvent(b, e),
            self._game_side_recheck(),
            b.update(),
        )
        ...
        self._spot_lbl.setToolTip(
            "Music source - click to open it; use the chevron to switch source"
        )
        self._spot_lbl.clicked.connect(self._on_source_click)
        self._spot_lbl._launchable = False
        self._spot_lbl._launch_glow = 0.4
        self._spot_lbl.enterEvent = lambda e, b=self._spot_lbl: (
            QPushButton.enterEvent(b, e),
            setattr(b, "_launch_glow", 1.0),
            self._apply_source_icon(),
            self._set_src_side("icon"),
            b.update(),
        )
        self._spot_lbl.leaveEvent = lambda e, b=self._spot_lbl: (
            QPushButton.leaveEvent(b, e),
            setattr(b, "_launch_glow", 0.4),
            self._apply_source_icon(),
            self._src_side_recheck(),
            b.update(),
        )
        ...
        self._spot_caret.paintEvent = lambda e, b=self._spot_caret: (
            self._paint_caret_btn(b, e, "_src_caret_angle")
        )
        self._spot_caret.enterEvent = lambda e, b=self._spot_caret: (
            QPushButton.enterEvent(b, e),
            self._set_src_side("caret"),
            b.update(),
        )
        self._spot_caret.leaveEvent = lambda e, b=self._spot_caret: (
            QPushButton.leaveEvent(b, e),
            self._src_side_recheck(),
            b.update(),
        )
        ...
        self._btn_prev = self._media_btn(
            "prev", lambda: _go("routed_prev", _mk.media_prev)(), _M
        )
        self._btn_play = self._media_btn(
            "play",
            lambda: _go("routed_playpause", _mk.media_playpause)(),
            _PM,
            white=True,
        )
        self._btn_next = self._media_btn(
            "next", lambda: _go("routed_next", _mk.media_next)(), _M
        )
        self._pm_size = _PM
        ...
        ml.addWidget(
            self._slider_row(
                ...
                _init_vol,
                self._on_volume,
                tip="Master music volume - the same one your volume buttons change. Applies right away.",
                key="vol",
            )
        )
        ml.addWidget(
            self._slider_row(
                ...
                0,
                100,
                int(self._cfg.unfocused_level * 100),
                self._on_unfocused,
                tip="Music volume when the game window isn't focused (alt-tabbed, minimised, etc.). 0 = muted.",
                key="unfocused",
            )
        )
        _menu_row = self._slider_row(
            "Menu/Pause Volume",
            0,
            100,
            int(self._cfg.menu_level * 100),
            self._on_menu,
            tip="Music volume in menus and when paused. 0 = muted.",
            key="menu",
        )
        ...
        self._themed.append(
            lambda: _style_expander(self._mixer_adv_btn, lambda: self._mixer_adv_open)
        )
        ...
        self._duck_row = self._slider_row(
            "Ducked Volume",
            0,
            100,
            int(self._cfg.duck_level * 100),
            self._on_duck,
            tip="Music volume while someone talks in-game.\nSpeech detection isn't perfect - it can be a little wonky. Tune the fade in Extras.",
            key="duck",
        )
        ...
        self._duck_cb.toggled.connect(
            lambda on, c=self._duck_cb: c.setText("Enabled" if on else "Enable")
        )
        ...
        self._sens_row = self._slider_row(
            "Speech sensitivity",
            0,
            100,
            _thresh_to_sens(self._cfg.vad_threshold),
            self._on_sensitivity,
            tip="How eagerly Segue ducks for speech.\nHigher = ducks on quieter or less-certain audio.\nLower = only clear speech.\nRaise it if it misses talking, lower it if it ducks on engine / SFX.",
            key="sens",
        )
        ...
        _fade_row = self._slider_row(
            "Fade length",
            100,
            2000,
            _ms_from_ramp(self._cfg.volume_ramp_in),
            self._on_fade,
            tip="How long music takes to fade in when it returns.",
            key="fade",
            fmt=lambda x: f"{x} ms",
        )
        ...
        self._ownvoice_cb.setEnabled(
            self._cfg.ducking_enabled
            and getattr(self._cfg, "duck_scope", "game") == "system"
        )
        self._mic_combo.setEnabled(
            self._ownvoice_cb.isEnabled() and self._ownvoice_cb.isChecked()
        )
        ...
        self._overlay_cb.setText(
            "Enabled" if self._overlay_cb.isChecked() else "Enable"
        )
        self._overlay_cb.toggled.connect(
            lambda on, c=self._overlay_cb: c.setText("Enabled" if on else "Enable")
        )
        ...
        self._always_on_cb.setToolTip(
            "Keep the overlay (album art) visible instead of fading out when nothing changes"
        )
        ...
        self._ingame_cb.setToolTip(
            "Only show the overlay while the game window is focused.\nHides it on the desktop / when you alt-tab out."
        )
        ...
        self._themed.append(
            lambda w=_ov_sep: w.setStyleSheet(
                f"background: {_c('border')}; border: none;"
            )
        )
        ...
        self._move_overlay_btn.setIcon(
            QIcon(_move_pixmap(self._move_sz, _c("text_hint")))
        )
        ...
        self._move_overlay_btn.setToolTip(
            "Move the overlay: grab and drag it anywhere on screen, then click again to finish. (Tip: double-click the overlay to move it too.)"
        )
        self._move_overlay_btn.clicked.connect(self._on_move_overlay)
        self._overlay_pos_picker = _OverlayPositionPicker(
            self._cfg.overlay_position,
            self._cfg.overlay_custom_x,
            self._cfg.overlay_custom_y,
        )
        ...
        self._overlay_size_reset.clicked.connect(
            lambda: self._overlay_size_slider.setValue(100)
        )
        btn_row = QHBoxLayout()
        ...
        rebind_btn.setToolTip(
            f"Using {_dev_label}. Click to view all controls and rebind buttons."
        )
        ...
        self._themed.append(
            lambda w=_rtxt: w.setStyleSheet(
                f"color: {_c('text')}; background: transparent;"
            )
        )
        _rdot = QLabel("·")
        ...
        self._themed.append(
            lambda w=_rdot: w.setStyleSheet(
                f"color: {_c('text_hint')}; background: transparent;"
            )
        )
        ...
        _rl.addStretch(1)
        self._pill_text_w = (
            QFontMetrics(self._hint_font).horizontalAdvance("Rebind controls")
            + _s(5)
            + QFontMetrics(_ui_font(21, QFont.Bold)).horizontalAdvance("·")
            + _s(5)
            + QFontMetrics(_rdev_font).horizontalAdvance(_dev_label)
        )
        ...
        self._lock_btn._off_icon = (
            QIcon(_unl) if _unl is not None else _kind_icon("lock", _c("text_hint"))
        )
        self._lock_btn.setIcon(self._lock_btn._off_icon)
        self._lock_btn.setToolTip(
            "Lock track-skipping (also toggled by the controller)"
        )
        ...
        for _i, _bh in enumerate((0.38, 0.72, 0.52, 0.86, 0.46)):
            ...
            _vp.drawRoundedRect(
                _x, (_s(40) - _hh) // 2, _bw_, _hh, _bw_ * 0.45, _bw_ * 0.45
            )
        _vp.end()
        ...
        self._viz_badge.setStyleSheet(
            f"background:#F26B1D; color:#141414; font-weight:800; font-size:{_s(9)}px; border-radius:{_s(6)}px; padding:{_s(1)}px {_s(6)}px;"
        )
        ...
        _disc_pm = _tinted(
            os.path.join(_ASSETS, "Discord_logo_blue.png"), _c("icon"), 16
        )
        ...
        _flbl.linkHovered.connect(
            lambda u: (
                _flbl.setText(_foot_html(u or None)),
                _flbl.setCursor(Qt.PointingHandCursor if u else Qt.ArrowCursor),
            )
        )
        ...
        if sys.platform == "win32":
            if not self._frame_applied:
                ...
                try:
                    ...
                    self._taskbar = None
                QTimer.singleShot(
                    1500, lambda: self._taskbar and self._taskbar.add("late")
                )
    ...
    def _show_whats_new(self, info=None, post_update: bool = False) -> None:
        ...
        _hstops = [
            c.strip()
            for c in (getattr(info, "color", "") or _ACCENT).split(",")
            if c.strip()
        ] or [_ACCENT]
        if len(_hstops) > 1:
            _hpts = ", ".join(
                f"stop:{i / (len(_hstops) - 1):.3f} {c}" for i, c in enumerate(_hstops)
            )
        ...
        _wn_title.setStyleSheet(
            f"background:{_hbg}; color:{_htxt}; padding:{_s(11)}px {_s(22)}px;"
        )
        ...
        def _text_label(_html_txt):
            _html = _theme_note_html(
                _localize_inline_images(_html_txt.replace("\n", "<br>"))
            )
        ...
        for _idx, _line in enumerate(_lines):
            ...
            if _mh:
                ...
                _hl.setStyleSheet(
                    f"color:{_c('text') if _is_title else _c('text_hint')};"
                )
                v.addWidget(_hl)
                v.addSpacing(_s(9))
                _rule = QFrame()
                ...
                _rule.setStyleSheet(
                    f"background:{_pc if _is_title else _c('border')}; border:none;"
                )
                v.addWidget(_rule)
                v.addSpacing(_s(12))
                continue
            ...
            if _mb:
                ...
                _bd.setFont(_bf)
                _bd.setStyleSheet(
                    f"background:{_bg}; color:{_fg}; border-radius:{_s(5)}px; padding:{_s(2)}px {_s(8)}px; margin-top:{_s(2)}px;"
                )
                _bd.setAlignment(Qt.AlignCenter)
                _bd.adjustSize()
                _indent = _bd.sizeHint().width() + _s(9)
                _inner = _theme_note_html(
                    _localize_inline_images(_mb.group(2).replace("\n", "<br>"))
                )
                _body = QLabel(
                    f"<div style='line-height:138%; text-indent:{_indent}px'>{_inner}</div>"
                )
        ...
        get = QPushButton(
            "Got it"
            if post_update
            else (getattr(info, "cta_label", "") or "Get it") + " →"
        )
        get.setObjectName("savebtn")
        get.setCursor(Qt.PointingHandCursor)
        if _active_theme() == "light":
            get.setStyleSheet(
                f"QPushButton#savebtn {{ background:{_hbg}; color:{_htxt}; border:none; border-radius:{_s(8)}px; padding:{_s(9)}px {_s(12)}px; font-weight:700; }}QPushButton#savebtn:hover {{ background:{_hbg}; }}"
            )
        if post_update:
        ...
        try:
            cap = int(
                QGuiApplication.primaryScreen().availableGeometry().height() * 0.85
            )
        except Exception:
            cap = _s(700)
    ...
    def _apply_banner_color(self, color: str) -> None:
        ...
        if len(stops) > 1:
            pts = ", ".join(
                (f"stop:{i / (len(stops) - 1):.3f} {c}" for i, c in enumerate(stops))
            )
    ...
    def _on_update_banner_click(self) -> None:
        ...
        url = (getattr(info, "installer_url", "") if info else "") or (
            info.ko_fi_url if info else ""
        )
        if url:
    ...
    def _on_update_progress(self, done: int, total: int) -> None:
        ...
        self._ub_btn.setText(
            "Downloading {}%".format(pct) if total > 0 else "Downloading…"
        )
    ...
    def _on_update_failed(self, msg: str) -> None:
        ...
        info = self._update_info
        self._ub_btn.setText(
            (getattr(info, "cta_label", "") or "Get it") + " →" if info else "Get it →"
        )
        return None
    ...
    def _preview_update_banner(self) -> None:
        ...
        if os.path.exists(_staged):
            try:
                ...
                pass
        _hero = os.path.normpath(
            os.path.join(
                _ASSETS, "..", "..", "docs", "release-assets", "whatsnew_121_forza.png"
            )
        )
        _duck_img = os.path.normpath(
            os.path.join(
                _ASSETS, "..", "..", "docs", "release-assets", "discord_ducking.png"
            )
        ).replace("\\", "/")
        _garage_img = os.path.normpath(
            os.path.join(
                _ASSETS, "..", "..", "docs", "release-assets", "garage_skipping.jpg"
            )
        ).replace("\\", "/")
        self._show_update_banner(
            UpdateInfo(
    ...
    def _build_titlebar(self) -> QWidget:
        ...
        self._menu.aboutToHide.connect(
            lambda: (
                self._menu_btn.setAttribute(Qt.WA_UnderMouse, False),
    ...
    def _cap_btn(self, kind: str, slot, close: bool = False) -> QPushButton:
        ...
        if close:
            ...
            b.enterEvent = lambda e, btn=b: (
                QPushButton.enterEvent(btn, e),
                btn.setIcon(btn._ico_hot),
            )
            b.leaveEvent = lambda e, btn=b: (
                QPushButton.leaveEvent(btn, e),
                btn.setIcon(btn._ico_rest),
            )
        else:
        ...
        if h > 0:
            ...
            try:
                ...
                if self.y() + h > avail.bottom() - _s(8):
                    self.move(
                        self.x(), max(avail.top() + _s(8), avail.bottom() - h - _s(8))
                    )
            except Exception:
                pass
        self.update()
    ...
    def _style_tabs(self):
        ...
        a = self._active_tab
        for name, btn, ic, icd in (
            ("mixer", self._tab_mixer, self._ic_mixer, self._ic_mixer_dull),
            ("extras", self._tab_extras, self._ic_extras, self._ic_extras_dull),
        ):
            active = a == name
    ...
    def _build_menu(self):
        ...
        _disc_pm = _tinted(
            os.path.join(_ASSETS, "Discord_logo_blue.png"), _c("icon_dim"), 20
        )
        if _disc_pm:
        ...
        self._icon_right_row(
            self._menu, "Preferences…", _gear_pm, self._open_preferences
        )

        if _is_dev():
            self._menu.addAction("Stream overlay…").triggered.connect(
                self._open_overlay_editor
            )
            self._act_demo = self._menu.addAction(
                "Demo mode    ✓" if self._cfg.demo_mode else "Demo mode"
            )
        ...
        _un_pm = _tinted(
            os.path.join(_ASSETS, "Uninstall icon v2 red.png"), _c("icon_dim"), 14
        )
        self._icon_right_row(self._menu, "Uninstall…", _un_pm, self._menu_uninstall)

        _q_pm = _tinted(
            os.path.join(_ASSETS, "on-off-button for hamburger menu.png"),
            _c("icon_dim"),
            14,
        )
        ...
        self._menu.aboutToShow.connect(
            lambda: (
                setattr(self, "_menu_outside_ticks", 0),
                self._menu_hover_timer.start(),
            )
        )
    ...
    def _on_update_click(self):
        ...
        if self._cfg.demo_mode:
            QTimer.singleShot(
                900,
                lambda: (self._preview_update_banner(), self._update_check_settle()),
            )
            return None
    ...
    def _update_check_settle(self):
        ...
        QTimer.singleShot(
            2600, lambda: (self._update_status.setText(""), self._verbtn.show_refresh())
        )
        return None
    ...
    def _rebuild_presets_menu(self):
        ...
        def _game_icon(game_key: str):
            ...
            if p and os.path.exists(p):
                return QIcon(
                    QPixmap(p).scaled(
                        _s(16), _s(16), Qt.KeepAspectRatio, Qt.SmoothTransformation
                    )
                )
            return None
        ...
        if rows:
            ...
            for name, tag in rows:
                ...
                load_btn.clicked.connect(
                    lambda _=False, n=name: (self._load_preset(n), _close_all())
                )
                del_btn = QPushButton()
                del_btn.setObjectName("presetdel")
    ...
    def _apply_preset(self, data: dict):
        ...
        self._sliders["fade"].setValue(
            max(100, min(2000, _ms_from_ramp(self._cfg.volume_ramp_in)))
        )
        ...
        if hasattr(self, "_overlay_size_slider"):
            self._overlay_size_slider.setValue(
                int(round(self._cfg.overlay_scale * 100))
            )
        if hasattr(self, "_only_cover_cb"):
            self._only_cover_cb.setChecked(self._cfg.overlay_compact)
        if hasattr(self, "_ingame_cb"):
            self._ingame_cb.setChecked(
                getattr(self._cfg, "overlay_in_game_only", False)
            )
    ...
    def _load_source_icons(self):
        ...
        self._browser_on = _tinted(
            os.path.join(_ASSETS, "browser_white.png"), _c("icon"), 28
        ) or _globe_pixmap(28)
        ...
        self._lm_on = _load_scaled(
            os.path.join(_ASSETS, "localmedia.png"), 28
        ) or _folder_pixmap(28)
        self._lm_off = _dim_pixmap(self._lm_on)
        self._tidal_on = _zoom_icon(
            os.path.join(_ASSETS, "tidal.png"), _s(28), 1.3
        ) or _app_icon("", "T", 28)
        self._tidal_off = _dim_pixmap(self._tidal_on)
        self._amazon_on = _load_scaled(
            os.path.join(_ASSETS, "amazonmusic.png"), 28
        ) or _app_icon("", "A", 28)
        self._amazon_off = _dim_pixmap(self._amazon_on)
        self._ytm_on = _load_scaled(
            os.path.join(_ASSETS, "ytmusic.png"), 28
        ) or _app_icon("", "Y", 28)
        self._ytm_off = _dim_pixmap(self._ytm_on)
        _link_col = "#9a9a98" if _active_theme() == "light" else _c("icon")
        self._link_on = (
            _tinted(_LINK, _link_col, 30)
            or _load_scaled(_LINK, 30)
            or _link_pixmap(True)
        )
        self._link_off = (
            _tinted(_LINK_BROKEN, _c("icon_dim"), 32)
            or _load_scaled(_LINK_BROKEN, 32)
            or _dim_pixmap(self._link_on)
        )
    ...
    def _load_play_icons(self):
        ...
        _play_src = _load_scaled(
            os.path.join(_ASSETS, "play icon.png"), int(_PM * 0.47)
        )
        if _play_src is not None and not _play_src.isNull():
            ...
            _q = QPainter(_pp)
            _q.drawPixmap(
                (_s(_PM) - _play_src.width()) // 2 + _s(1),
                (_s(_PM) - _play_src.height()) // 2,
                _play_src,
            )
            _q.end()
        ...
        _pause_src = _load_scaled(
            os.path.join(_ASSETS, "pause icon v2.png"), int(_PM * 0.45)
        )
        if _pause_src is not None and not _pause_src.isNull():
            ...
            _qz = QPainter(_pz)
            _qz.drawPixmap(
                (_s(_PM) - _pause_src.width()) // 2,
                (_s(_PM) - _pause_src.height()) // 2,
                _pause_src,
            )
            _qz.end()
    ...
    def _retheme(self):
        ...
        try:
            ...
            _unl = _tinted(
                os.path.join(_ASSETS, "unlocked lock.png"), _c("text_hint"), 19
            )
            self._lock_btn._off_icon = (
                QIcon(_unl) if _unl is not None else _kind_icon("lock", _c("text_hint"))
            )
            ...
            self._lock_btn.setIcon(
                self._lock_btn._on_icon
                if self._lock_btn.property("active") == "true"
                else self._lock_btn._off_icon
            )
            ...
            for _i, _bh in enumerate((0.38, 0.72, 0.52, 0.86, 0.46)):
                ...
                _hh = int(_s(40) * _bh * 0.78)
                _vp.drawRoundedRect(
                    _x, (_s(40) - _hh) // 2, _bw, _hh, _bw * 0.45, _bw * 0.45
                )
            _vp.end()
            ...
            if getattr(self, "_overlay_size_reset", None) is not None:
                self._overlay_size_reset.setIcon(
                    QIcon(_undo_pixmap(20, _c("text_hint")))
                )
            ...
            if getattr(self, "_btn_min", None) is not None:
                ...
                self._btn_max.setIcon(
                    _caption_icon("restore" if self.isMaximized() else "max")
                )
                self._btn_close._ico_rest = _caption_icon("close")
                self._btn_close.setIcon(self._btn_close._ico_rest)
            if getattr(self, "_foot_lbl", None) is not None and getattr(
                self, "_foot_html", None
            ):
            ...
            self._brand_logo = QPixmap(
                os.path.join(_ASSETS, "Segue logo in bottom ui.png")
            )
            self._brand_pm = None
            ...
            if (
                not getattr(self, "_last_thumb", None)
                and getattr(self, "_cover", None) is not None
            ):
                self._cover.setPixmap(_cover_placeholder(self._cover_sz, _s(8)))
    ...
    def _refresh_controls_pill(self):
        ...
        if getattr(self, "_rdev_lbl", None) is not None:
            ...
            self._pill_text_w = (
                ...
                + QFontMetrics(_ui_font(21, QFont.Bold)).horizontalAdvance("·")
                + _s(5)
                + QFontMetrics(_bf).horizontalAdvance(_dev_name(dev))
            )
        btn = getattr(self, "_controls_btn", None)
        if btn is not None:
            btn.setToolTip(
                f"Using {_dev_name(dev)}. Click to view all controls and rebind buttons."
            )
            btn.update()
            return None
    ...
    def _open_help(self):
        ...
        sig = (dev, mode, preset)
        if (
            getattr(self, "_help", None) is None
            or getattr(self, "_help_sig", None) != sig
        ):
            if getattr(self, "_help", None) is not None:
                try:
                    ...
                    pass
            self._help = _HelpWindow(
                input_device=dev,
                mode=mode,
                on_open_controls=self._open_rebind,
                preset=preset,
            )
            self._help_sig = sig
        self._help.show()
    ...
    def _maybe_run_tour(self, force: bool = False):
        ...
        self._tour = _TourOverlay(
            self,
            self._tour_steps(),
            self._mark_tour_done,
            on_finish_cta=self._open_setup_help if _is_forza else None,
            final_cta_label="Done",
            final_secondary_label="Setup" if _is_forza else "Skip",
        )
        return None
    ...
    def _tour_steps(self):
        return [
            (
                self._conn_pane,
                "Connections",
                "Game + music status. Both turn green when connected.<br><br>◂ next to the game icon = switch game.<br>▾ next to the source icon = switch music source.",
            ),
            (
                self._np_area,
                "Now playing",
                "Your current track + cover art.<br><br>Click the source icon up top to switch Spotify ↔ browser.",
            ),
            (
                [self._tab_mixer, self._tab_extras],
                "Mixer & Extras",
                "<b>Mixer</b>: the volume sliders.<br><br><b>Extras</b>: fade length, overlay, Open-with games.",
            ),
            (
                [self._btn_prev, self._btn_play, self._btn_next],
                "Media transport",
                "Previous / play-pause / next.<br><br>Drives Spotify (or your browser) via Windows media keys.",
            ),
            (
                [self._controls_btn, self._lock_btn, self._power_btn],
                "Bottom controls",
                "<b>Controls</b>: pick and rebind your device.<br><b>Lock Skip</b>: freeze track-skipping.<br><b>Power</b>: pause Segue without closing.",
            ),
    ...
    def _confirm_quit(self) -> bool:
        ...
        dlg = _ConfirmDialog(
            self,
            "Quit Segue?",
            "This stops Segue controlling your music and restores Spotify's volume.",
            confirm_text="Quit Segue",
            cancel_text="Cancel",
            show_dont_ask=True,
        )
    ...
    def _menu_uninstall(self):
        ...
        if exe_dir:
            candidates = [
                os.path.join(exe_dir, "_uninstall", "unins000.exe"),
                os.path.join(exe_dir, "unins000.exe"),
            ]
        ...
        if not unins:
            dlg = _ConfirmDialog(
                ...
                body="No installer found. You're running Segue from source - uninstall is only available in the installed build.",
                confirm_text="OK",
                cancel_text="",
            )
            dlg.exec()
        ...
        dlg = _ConfirmDialog(
            self,
            title="Uninstall Segue?",
            body="This will close Segue and launch the uninstaller. Your settings (in %APPDATA%\\Segue) will also be removed.",
    ...
    def _notify_tray_once(self):
        ...
        try:
            ...
            if tray is not None:
                tray.showMessage(
                    "Segue is still running",
                    "Closed to the tray, your music keeps mixing. Right-click the tray icon to quit.",
    ...
    def _spinner_targets(self):
        ...
        return (
            (getattr(self, "_spot_lbl", None), self._apply_source_icon),
            (getattr(self, "_forza_lbl", None), self._apply_game_icon),
        )
    ...
    def _stop_launch_spinner(self, b=None, apply=None):
        ...
        if not any(
            (
                (x is not None) and getattr(x, "_launching", False)
                for x, _ in self._spinner_targets()
            )
        ):
            if getattr(self, "_launch_spin_timer", None) is not None:
                self._launch_spin_timer.stop()
    ...
    def _apply_source_icon(self):
        ...
        if getattr(b, "_launching", False):
            ...
            b.setIcon(
                _launcher_icon(
                    src, box, spinner=True, angle=getattr(b, "_spin_angle", 0), dx=_dx
                )
            )
            return None
    ...
    def _apply_game_icon(self):
        ...
        if getattr(b, "_launching", False):
            b.setIcon(
                _launcher_icon(
                    base, box, spinner=True, angle=getattr(b, "_spin_angle", 0)
                )
            )
            return None
        if getattr(b, "_launchable", False) and b.underMouse():
    ...
    def _animate_launch_glow(self, b, target):
        ...
        anim.valueChanged.connect(
            lambda v, bb=b: (
                setattr(bb, "_launch_glow", float(v)),
                self._apply_source_icon(),
            )
        )
        anim.start()
    ...
    def _refresh_status(self):
        ...
        if _src == "browser":
            ...
            _cpm = (
                _exe_icon(_cip, "C", 26).pixmap(_s(26), _s(26))
                if _cip
                else _custom_glyph_pixmap(26)
            )
        ...
        if general_mode:
            ...
            if preset_icon_path:
                pm = QPixmap(preset_icon_path).scaled(
                    _s(26), _s(26), Qt.KeepAspectRatio, Qt.SmoothTransformation
                )
        ...
        game_label = (
            _pretty(self._cfg.general_target_process) if general_mode else "Forza"
        )
        if general_mode:
        ...
        self._np_artist.setCursor(
            _np_cur if self._np_clickable and self._np_raw_artist else Qt.ArrowCursor
        )
        ...
        if pk is not None:
            ...
            if (cx, cy) != (
                pk._cx,
                pk._cy,
            ) or pk._current != self._cfg.overlay_position:
                pk._cx, pk._cy = cx, cy
    ...
    def _set_move_mode(self, on: bool):
        ...
        if self._ui is not None:
            ...
            self._ui["overlay_ping"] = time.monotonic()
        self._move_overlay_btn.setIcon(
            QIcon(_move_pixmap(self._move_sz, _ACCENT if on else _c("text_hint")))
        )
        return None
    ...
    def _update_overlay_subs_enabled(self):
        ...
        for w in (
            ...
            self._ingame_cb,
            self._drive_cb,
            self._overlay_pos_picker,
            self._overlay_size_slider,
            self._overlay_size_val,
            self._overlay_size_reset,
        ):
            w.setEnabled(on)
        return None
    ...
    def _on_ducking(self, on):
        ...
        if hasattr(self, "_ownvoice_cb"):
            self._ownvoice_cb.setEnabled(
                bool(on) and getattr(self._cfg, "duck_scope", "game") == "system"
            )
        if hasattr(self, "_mic_row"):
            self._mic_combo.setEnabled(
                self._ownvoice_cb.isEnabled() and self._ownvoice_cb.isChecked()
    ...
    def _media_btn(
        self, kind: str, slot, size: int, white: bool = False
    ) -> QPushButton:
        b = QPushButton()
    ...
    def _attach_press_bounce(self, btn, dip=0.9):
        ...
        def _pe(e, b=btn):
            ...
            if getattr(b, "_ic_key", None) != key:
                ...
                if tc is None or lc is None:
                    cr = b.style().subElementRect(
                        QStyle.SubElement.SE_PushButtonContents, opt, b
                    )
                    tc = (
                        QRectF(cr).center()
                        if cr.isValid()
                        else QRectF(b.rect()).center()
                    )
                    lc = tc
                b._ic_tc = tc
        ...
        anim.finished.connect(
            lambda b=btn: (
                setattr(b, "_press_dim", 0.0),
                setattr(b, "_press_scale", 1.0),
                b.update(),
            )
        )
    ...
    def _paint_caret_btn(self, b, e, angle_attr, side_attr="_src_hover_side"):
        ...
        _lit = (
            b.underMouse()
            or b.property("menuopen") == "true"
            or getattr(self, side_attr, None) is not None
        )
    ...
    def _rotate_caret(self, angle_attr, btn, to_angle):
        ...
        anim.valueChanged.connect(
            lambda v, b=btn, a=angle_attr: (setattr(self, a, float(v)), b.update())
        )
        anim.start()
    ...
    def _set_src_side(self, side):
        self._set_split_side(
            getattr(self, "_src_box", None),
            getattr(self, "_spot_lbl", None),
            getattr(self, "_spot_caret", None),
            "_src_hover_side",
            side,
        )
        return None
    ...
    def _set_game_side(self, side):
        self._set_split_side(
            getattr(self, "_game_box", None),
            getattr(self, "_forza_lbl", None),
            getattr(self, "_game_caret", None),
            "_game_hover_side",
            side,
        )
        return None
    ...
    def _browser_icon_for(self, appid: str):
        ...
        if not friendly or friendly in (
            "Spotify",
            "Apple Music",
            "Media Player",
            "VLC",
        ...
        pm = None
        asset = os.path.join(
            _ASSETS, "browser_" + friendly.lower().replace(" ", "") + ".png"
        )
    ...
    def _game_menu(self):
        ...
        for key in _gp.GAME_PRESETS:
            ...
            if key == "other":
                ...
                label = (
                    f"Other game ({cur_target})"
                    if cur_target and current == "other"
                    else "Other game…"
                )
            icon_path = ""
            if key == "forza":
                ...
                if os.path.exists(cand):
                    icon_path = cand
            if key == "forza":
                pm = _forza_pixmap(22) or QPixmap(_FORZA).scaled(
        ...
        auto_cb.setStyleSheet(
            f"QCheckBox {{ color: {_c('text')}; padding: {_s(8)}px {_s(12)}px; }}"
        )
        ...
        popup.closed.connect(
            lambda: (
                setattr(self, "_game_menu_closed_at", time.monotonic()),
                setattr(self, "_game_popup", None),
                self._set_caret_open(False, "_game_caret", "_game_box"),
                self._game_side_recheck(),
                self._rotate_caret("_game_caret_angle", self._game_caret, 0.0),
            )
        )
    ...
    def _apply_preset_visibility(self) -> None:
        ...
        if hasattr(self, "_sliders"):
            ...
            if unfoc_slider is not None:
                unfoc_slider.parent().setVisible(
                    _gp.show_control(key, "unfocused_volume")
                )
    ...
    def _source_menu_obj(self):
        ...
        if _cache is None:
            ...
            browser_pm = _tinted(
                os.path.join(_ASSETS, "browser_white.png"), _c("icon"), 22
            ) or _globe_pixmap(22)
            am_pm = _scaled(
                os.path.join(_ASSETS, "applemusic.png"), _app_icon("", "M", 22)
            )
            lm_pm = _scaled(os.path.join(_ASSETS, "localmedia.png"), _folder_pixmap(22))
            _td = os.path.join(_ASSETS, "tidal.png")
            td_pm = (
                _zoom_icon(_td, sz, 1.32)
                if os.path.exists(_td)
                else _app_icon("", "T", 22).scaled(
                    sz, sz, Qt.KeepAspectRatio, Qt.SmoothTransformation
                )
            )
            amz_pm = _scaled(
                os.path.join(_ASSETS, "amazonmusic.png"), _app_icon("", "A", 22)
            )
            _ytm = os.path.join(_ASSETS, "ytmusic.png")
            ytm_pm = (
                _zoom_icon(_ytm, sz, 1.18)
                if os.path.exists(_ytm)
                else _app_icon("", "Y", 22).scaled(
                    sz, sz, Qt.KeepAspectRatio, Qt.SmoothTransformation
                )
            )
            _cache = self._src_menu_pm = {
                "spot": spot_pm,
                "browser": browser_pm,
                "am": am_pm,
                "lm": lm_pm,
        ...
        _cust_icon = (
            _exe_icon(_cust_path, "C", 22)
            if _cust_path
            else QIcon(_custom_glyph_pixmap(22))
        )
        cust_act = menu.addAction(
            _cust_icon, "Custom…" + ("    ✓" if cur == "custom" else "")
        )
        ...
        menu.aboutToShow.connect(
            lambda: (
                self._set_caret_open(True),
                self._rotate_caret("_src_caret_angle", self._spot_caret, 180.0),
            )
        )
        menu.aboutToHide.connect(
            lambda: (
                setattr(self, "_spot_menu_closed_at", time.monotonic()),
                self._set_caret_open(False),
                self._src_side_recheck(),
                self._rotate_caret("_src_caret_angle", self._spot_caret, 0.0),
            )
        )
    ...
    def _launch_source(self):
        ...
        msg = {
            "running": f"{r.display} is already open",
            "launched": f"Opening {r.display}…",
            "unsupported": f"Open {r.display} manually to start playback",
            "not_found": f"Couldn't find {r.display}. Open it manually.",
        }.get(r.status, "")
        if msg:
    ...
    def _source_toast(self, text: str):
        ...
        try:
            ...
            if tray is not None:
                tray.showMessage(
                    "Segue", text, QSystemTrayIcon.MessageIcon.Information, 3000
                )
                return None
        except Exception:
            return None
