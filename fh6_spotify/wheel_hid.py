...
from fh6_spotify.input_backend import effective_bindings, evaluate_binds, open_gesture, vol_delta
...
class _HIDP_CAPS(ctypes.Structure):
    _fields_ = [('Usage', ctypes.c_ushort), ('UsagePage', ctypes.c_ushort), ('InputReportByteLength', ctypes.c_ushort), ('OutputReportByteLength', ctypes.c_ushort), ('FeatureReportByteLength', ctypes.c_ushort), ('Reserved', ctypes.c_ushort * 17), ('NumberLinkCollectionNodes', ctypes.c_ushort), ('NumberInputButtonCaps', ctypes.c_ushort), ('NumberInputValueCaps', ctypes.c_ushort), ('NumberInputDataIndices', ctypes.c_ushort), ('NumberOutputButtonCaps', ctypes.c_ushort), ('NumberOutputValueCaps', ctypes.c_ushort), ('NumberOutputDataIndices', ctypes.c_ushort), ('NumberFeatureButtonCaps', ctypes.c_ushort), ('NumberFeatureValueCaps', ctypes.c_ushort), ('NumberFeatureDataIndices', ctypes.c_ushort)]


class _CAPS_RANGE(ctypes.Structure):
    _fields_ = [('UsageMin', ctypes.c_ushort), ('UsageMax', ctypes.c_ushort), ('StringMin', ctypes.c_ushort), ('StringMax', ctypes.c_ushort), ('DesignatorMin', ctypes.c_ushort), ('DesignatorMax', ctypes.c_ushort), ('DataIndexMin', ctypes.c_ushort), ('DataIndexMax', ctypes.c_ushort)]


class _HIDP_VALUE_CAPS(ctypes.Structure):
    _fields_ = [('UsagePage', ctypes.c_ushort), ('ReportID', ctypes.c_ubyte), ('IsAlias', ctypes.c_ubyte), ('BitField', ctypes.c_ushort), ('LinkCollection', ctypes.c_ushort), ('LinkUsage', ctypes.c_ushort), ('LinkUsagePage', ctypes.c_ushort), ('IsRange', ctypes.c_ubyte), ('IsStringRange', ctypes.c_ubyte), ('IsDesignatorRange', ctypes.c_ubyte), ('IsAbsolute', ctypes.c_ubyte), ('HasNull', ctypes.c_ubyte), ('Reserved', ctypes.c_ubyte), ('BitSize', ctypes.c_ushort), ('ReportCount', ctypes.c_ushort), ('Reserved2', ctypes.c_ushort * 5), ('UnitsExp', ctypes.c_ulong), ('Units', ctypes.c_ulong), ('LogicalMin', ctypes.c_long), ('LogicalMax', ctypes.c_long), ('PhysicalMin', ctypes.c_long), ('PhysicalMax', ctypes.c_long), ('Range', _CAPS_RANGE)]


class _OVERLAPPED(ctypes.Structure):
    _fields_ = [('Internal', ctypes.c_void_p), ('InternalHigh', ctypes.c_void_p), ('Offset', wintypes.DWORD), ('OffsetHigh', wintypes.DWORD), ('hEvent', ctypes.c_void_p)]


_HAT_XY = {0: (0, 1), 1: (1, 1), 2: (1, 0), 3: (1, -1), 4: (0, -1), 5: (-1, -1), 6: (-1, 0), 7: (-1, 1)}
...
class _HidDevice:
    ...
    def __init__(self, path: bytes, name: str):
        ...
        self._stop = False
        self._thread = None
        self._h = _k32.CreateFileW(path.decode('utf-8', 'ignore'), _GENERIC_READ, _FILE_SHARE_READ | _FILE_SHARE_WRITE, None, _OPEN_EXISTING, _FILE_FLAG_OVERLAPPED, None)
        ...
        self.button_space = int(_hid.HidP_MaxUsageListLength(_HIDP_INPUT, _USAGE_PAGE_BUTTON, self._pp)) or 0
        self._hat_caps = []
        self._logged_events = set()
        n = ctypes.c_ushort(caps.NumberInputValueCaps)
        if n.value:
            arr = (_HIDP_VALUE_CAPS * n.value)()
            if _hid.HidP_GetValueCaps(_HIDP_INPUT, arr, ctypes.byref(n), self._pp) == _HIDP_STATUS_SUCCESS:
                for i in range(n.value):
                    vc = arr[i]
                    usage = vc.Range.UsageMin if vc.IsRange else vc.Range.UsageMin
                    if vc.UsagePage == _USAGE_PAGE_GENERIC and usage == _USAGE_HATSWITCH:
                        self._hat_caps.append((int(vc.ReportID), int(vc.LogicalMin)))
        self.hat_count = len(self._hat_caps)
        self.alive = True
        self._thread = threading.Thread(target=self._run, daemon=True, name=f'wheel-hid:{self.name}')
        self._thread.start()
    ...
    def _run(self):
        ...
        try:
            while not self._stop:
                ...
                ov.hEvent = ev
                ok = _k32.ReadFile(self._h, buf, self._report_len, None, ctypes.byref(ov))
                ...
                if not _k32.GetOverlappedResult(self._h, ctypes.byref(ov), ctypes.byref(got), False):
                    break
                if got.value:
                    self._parse(buf)
        finally:
    ...
    def _parse(self, buf):
        ...
        if max_u:
            ...
            st = _hid.HidP_GetUsages(_HIDP_INPUT, _USAGE_PAGE_BUTTON, 0, usages, ctypes.byref(ulen), self._pp, buf, self._report_len)
        ...
        for slot, (rid, lmin) in enumerate(self._hat_caps):
            ...
            st = _hid.HidP_GetUsageValue(_HIDP_INPUT, _USAGE_PAGE_GENERIC, 0, _USAGE_HATSWITCH, ctypes.byref(val), self._pp, buf, self._report_len)
            if st == _HIDP_STATUS_SUCCESS:
...
class WheelHidInput:
    ...
    def poll(self, now: float, can_skip: bool) -> float:
        ...
        binds = effective_bindings(self.c.input_device, self.c.bindings, getattr(self.c, 'mode', None))
    ...
    def read_pressed(self):
        ...
        if not down:
            return
        btns = sorted((c for c in down if c.startswith('btn:')), key=lambda c: int(c.split(':')[1]))
        if btns:
            return btns[0]
