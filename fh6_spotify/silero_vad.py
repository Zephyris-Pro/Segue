...
class SileroClassifier:
    ...
    def __call__(self, frame_bytes: bytes) -> bool:
        ...
        if self._pending_len < _CHUNK:
            ...
            self._buf = np.concatenate(self._pending) if len(self._pending) > 1 else self._pending[0]
            self._pending = None
            while len(self._buf) >= _CHUNK:
                chunk = self._buf[:_CHUNK]
                self._buf = self._buf[_CHUNK:]
                x = np.concatenate([self._context, chunk.reshape(1, _CHUNK)], axis=1).astype(np.float32)
                candidate_feeds = {'input': x, 'state': self._state, 'sr': self._sr}
                feeds = {k: v for k, v in candidate_feeds.items() if k in self._in_names}
