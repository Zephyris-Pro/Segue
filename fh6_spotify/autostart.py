...
def _startup_lnk() -> str:
    ...
    return os.path.join(base, 'Microsoft', 'Windows', 'Start Menu', 'Programs', 'Startup', 'Segue.lnk')
...
def installed_mode():
    ...
    if not os.path.exists(lnk):
        ...
        try:
            ps = '$s=(New-Object -ComObject WScript.Shell).CreateShortcut(\'{}\');Write-Output $s.Arguments'.format(lnk)
            out = subprocess.run(['powershell', '-NoProfile', '-Command', ps], capture_output=True, text=True, creationflags=_CREATE_NO_WINDOW, timeout=10).stdout or ''
...
def _target_args_workdir(direct=False):
    ...
    if getattr(sys, 'frozen', False):
        return (sys.executable, '' if direct else '--watch', os.path.dirname(sys.executable))
    else:
...
def _kill_watchers() -> None:
    ...
    for proc in psutil.process_iter(['pid', 'cmdline']):
        try:
            ...
            if proc.pid!= me and '--watch' in cl and any(('fh6_spotify' in c or 'Segue' in c for c in cl)):
                        proc.terminate()
...
def install(direct: bool=False) -> None:
    ...
    ps = '$s=(New-Object -ComObject WScript.Shell).CreateShortcut(\'{lnk}\');$s.TargetPath=\'{t}\';$s.Arguments=\'{a}\';$s.WorkingDirectory=\'{w}\';$s.Save()'.format(lnk=lnk, t=target, a=args, w=workdir)
    try:
        subprocess.run(['powershell', '-NoProfile', '-Command', ps], creationflags=_CREATE_NO_WINDOW, timeout=10)
    except Exception as exc:
