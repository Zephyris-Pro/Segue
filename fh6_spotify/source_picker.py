...
def resolve_pick(app: AudioApp, smtc_app_ids) -> dict:
    ...
    return {'custom_process_names': (app.exe,), 'custom_smtc_match': match_smtc(app.exe, smtc_app_ids), 'custom_label': (app.display or '').strip() or _exe_base(app.exe) or 'Custom', 'custom_icon_path': getattr(app, 'path', '') or ''}
def builtin_source_exes(config) -> set:
    ...
    names = set()
    for src in ['spotify', 'browser', 'applemusic', 'localmedia', 'tidal', 'amazonmusic', 'ytmusic']:
        ...
        try:
            if not s.Process:
                ...
                if not exe or exe.lower() in seen or exe.lower() in skip:
                    ...
                    rows.append((peak if peak is not None else 0.0, AudioApp(exe, disp, path)))
        except Exception:
            pass
