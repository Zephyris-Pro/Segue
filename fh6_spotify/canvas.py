...
def update_np_video(
    title: str, artist: str, ui: dict, enabled: bool, port: int
) -> None:
    ...
    if not enabled:
        ...
        if not title:
            ...
            threading.Thread(
                target=_resolve, args=(title, artist, key, ui, port), daemon=True
            ).start()
...
def _resolve(title: str, artist: str, key: str, ui: dict, port: int) -> None:
    ...
    try:
        ...
        req = urllib.request.Request(
            f"http://127.0.0.1:{port}/canvas?{q}", headers={"User-Agent": "Segue"}
        )
