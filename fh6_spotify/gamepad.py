...
def touch_volume_delta(
    cur_y: int, prev_y: int, active: bool, was_active: bool, sensitivity: float
) -> float:
...
def is_tap(
    duration_ms: float, movement: int, max_ms: int, max_move: int, min_ms: float = 0.0
) -> bool:
...
def classify_swipe(
    dx: int, dy: int, swipe_threshold: int, vol_deadzone: int = 25
) -> str | None:
    ...
    for product in _DS_PRODUCTS:
        try:
            return hidapi.Device(
                vendor_id=_DS_VENDOR, product_id=product, blocking=False
            )
